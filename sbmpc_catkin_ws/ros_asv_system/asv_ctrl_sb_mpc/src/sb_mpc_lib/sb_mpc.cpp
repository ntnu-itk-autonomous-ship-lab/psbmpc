...
simulationBasedMpc::simulationBasedMpc(){
  ...
  T_stat_ = 60.0;		  // (50.0)
	...
	D_INIT_ = 700;//1852.0;	  // should be >= D_CLOSE 300.0 600.0 500.0 700.0 800 1852
	D_CLOSE_ = 500;//1000.0;	// 200.0 300.0 400.0 500.0 600 1000
	D_SAFE_ = 300; //185.2; 	  // 40.0, 50.0, 70.0, 80.0, 100, 200, 185.2
	...
	LOS_LA_DIST_ = 145.0; 	// LOS lookahead distance (100.0) (********************)
...
void simulationBasedMpc::getBestControlOffset(
	double &u_os_best, 
	double &psi_os_best, 
	double u_d, double psi_d_, 
	const Eigen::Matrix<double,6,1>& asv_state, 
	const Eigen::Matrix<double,-1,10>& obst_states, 
	const Eigen::Matrix<double,-1,4>& static_obst, 
	const Eigen::Matrix<double,-1,2>& waypoints_, 
	Eigen::Matrix<double,-1,2>& predicted_traj, 
	Eigen::Matrix<double,-1,1>& colav_status, 
	Eigen::Matrix<double,-1,-1>& obst_status){
		    ...
		    // Simulate ASV trajectory for current control behavior
				...
				for (int k = 0; k < n_obst; k++){
					...
					for (int l = 0; l < n_obst_branches; l++){
						...
						cost_k = costFunction(P_ca_[j], Chi_ca_[i], k, (bool)SB_0(k), (bool)AH_0(k), (bool)CRG_0(k), (bool)OTG_0(k), (bool)OT_0(k), (bool)HOT_0(k), (bool)OBS_PASSED_0(k), DIST_0(k), u_d, l, static_obst, ik_return_to_path);
...
double simulationBasedMpc::costFunction(
	double P_ca, 
	double Chi_ca, 
	int k, 
	bool SB_0, 
