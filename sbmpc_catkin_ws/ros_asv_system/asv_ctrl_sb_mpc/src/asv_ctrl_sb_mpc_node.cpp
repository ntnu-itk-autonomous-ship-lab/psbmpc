...
int main(int argc, char *argv[])
{
	...
	simulationBasedMpcRosLink *sb_mpc_link = new simulationBasedMpcRosLink; //RosLink
	...
	ros::Subscriber wp_sub = n.subscribe("asv/next_waypoint",
							1,
							&simulationBasedMpcNode::wpCallback,
							&sb_mpc_node);
	...
	sb_mpc_node.initialize(&cmd_pub, & os_pub, &obstacle_sub, &og_sub, &asv_sub, &cmd_sub, &wp_sub, sb_mpc_link); //RosLink
	sb_mpc_node.start();
...
void simulationBasedMpcNode::initialize(ros::Publisher *cmd_pub,
					...
					simulationBasedMpcRosLink *sb_mpc_link) //RosLink
{
	cmd_pub_ = cmd_pub;
	os_pub_ = os_pub;
	...
	sb_mpc_ = sb_mpc_link; //RosLink
	u_os_ = 1;
	psi_os_ = 0;
	
	sb_mpc_->initialize(&obstacles_, &next_waypoint_, &map_); //RosLink
}										

void simulationBasedMpcNode::start()
{
	double rate = 10.0; // 10.0
	...
	while (ros::ok())
	{
		...
		if (t > 5){
			tick = clock();
			sb_mpc_->getBestControlOffset(u_os_, psi_os_); //RosLink does not have this fxn!
			tock = clock() - tick;
			t = 0;
			os_.P_ca = u_os_; 
			os_.Chi_ca = psi_os_; // NB! negative for MR interface
			os_pub_->publish(os_);
			ROS_INFO("Runtime: %0.2f", ((float)tock)/CLOCKS_PER_SEC);
			
			// test print
			//ROS_INFO("next_waypoint_ size: %0.2f   ", (double)next_waypoint_.size());
			//ROS_INFO("obstacles_ size: %0.2f   ", (double)obstacles_.size());
			//ROS_INFO("psi_d_: %0.2f   ", psi_d_);
		}
		t += 1/rate;

		cmd_vel_.linear.x = u_d_*u_os_;
		cmd_vel_.angular.y = psi_d_ + psi_os_; // NB! negative for MR interface
...
{
	sb_mpc_->updateAsvState(msg, u_d_, psi_d_, &next_waypoint_); //RosLink
}
...
void simulationBasedMpcNode::wpCallback(const asv_msgs::WPArray::ConstPtr &msg)
{	
	next_waypoint_ = msg->wp_xy;
