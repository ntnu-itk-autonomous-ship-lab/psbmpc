...
class LOSGuidanceROS(object):
    ...
    def __init__(self,
        ...
        self._nwp_publisher   = rospy.Publisher("next_waypoint", asv_msgs.msg.WPArray, queue_size=1)
    	self.wparray = asv_msgs.msg.WPArray()
    	self.wparray.wp_xy.append(asv_msgs.msg.WP()) # one WP (i.e. nwp) added, do we need more? 
    	self.wparray.wp_xy.append(asv_msgs.msg.WP()) # one WP (i.e. nwp) added, do we need more? 
    	self.wparray.wp_xy.append(asv_msgs.msg.WP()) # one WP (i.e. nwp) added, do we need more? 
    ...
    def _visualize_waypoints(self, switched):
        ...
        if self._first_draw:
            for wp in range(1, self.nwp):
                ...
                #self._nwp_publisher.publish(self.wp)
    ...
    def _update(self):
        ...
        u_d, psi_d, switched, xk0, yk0, xk1, yk1, xk2, yk2 = self.controller.update(self.odom.pose.pose.position.x,
        ...
        # Publish current and next waypoints
        self.wparray.wp_xy[0].x = xk0
        self.wparray.wp_xy[0].y = yk0
        
        self.wparray.wp_xy[1].x = xk1
        self.wparray.wp_xy[1].y = yk1
        
        self.wparray.wp_xy[2].x = xk2
        self.wparray.wp_xy[2].y = yk2
    	
    	self._nwp_publisher.publish(self.wparray)
        
        #print "next Waypoint (%.2f, %.2f)" % (self.wparray.wp_xy[0].x, self.wparray.wp_xy[0].y)
...
class LOSGuidance(Controller):
    ...
    def __init__(self,
        ...
        self.cWP = 0 # Current waypoint index
        self.wp = None
    ...
    def progress_along_path(self, x, y): # not correct! should include total track distance
        return \
    ...
    def update(self, x, y):
        ...
        if self.R2 > 999999:
            ...
            xk0 = self.wp[self.cWP][0] 
            xk1=xk0 
            xk2=xk0 
            
            yk0 = self.wp[self.cWP][1] 
            yk1=yk0 
            yk2=yk0   
                
            #return 0.001, 1.57, False # 0, self.Xp, False 
	    return 0.001, 1.57, False, xk0, yk0, xk1, yk1, xk2, yk2  
        ...
        if self.switching_criterion(x, y):
            while self.switching_criterion(x,y):
                ...
                # There are still waypoints left
                    ...
                    #xk1 = self.wp[self.cWP + 1][0]
        	    #yk1 = self.wp[self.cWP + 1][1] 
        	    #print "Waypoint: (%.2f, %.2f) is next!" % (xk1,yk1)

                    if (np.abs(new_course - self.Xp) > np.pi/4.0):
                        self.e_integral = 0.0

                    self.Xp = new_course
                    self.cWP += 1 # index of next waypoint
                    switched = True
                else:
        ...
        # next waypoint, or current waypoint if last waypoint reached
        xk1 = self.wp[self.cWP][0]
        yk1 = self.wp[self.cWP][1] 


	# current waypoint
	if self.cWP > 0:
            xk0 = self.wp[self.cWP-1][0]
            yk0 = self.wp[self.cWP-1][1]  
        else:
            # current waypoint is last waypoint
            xk0 = xk1
            yk0 = yk1 
        ...
        # next next waypoint
        if self.cWP < self.nWP - 1:
            xk2 = self.wp[self.cWP+1][0]
            yk2 = self.wp[self.cWP+1][1] 
        else:
            # no next next waypoint
            xk2 = xk1
            yk2 = yk1 
        
        
        
        #wparray.wp_xy[0].x = xk
        #wparray.wp_xy[0].y = yk
        
    	#nwp_publisher.publish(wparray)
        
        # copy next waypoint, or current waypoint if last waypoint reached 
	xk = xk1
	yk = yk1
        ...
        return self.u_d, psi_d, switched, xk0, yk0, xk1, yk1, xk2, yk2
...
if __name__ == "__main__":
    ...
    R2 = rospy.get_param("~acceptance_radius", 20)**2 #20 150
    dt = rospy.get_param("~update_rate", .2)
    de = rospy.get_param("~lookahead_distance", 100.0) # 40.0 60.0 80.0 (100.0) 150.0
    ...
    #nwp_publisher   = rospy.Publisher("next_waypoint", asv_msgs.msg.WPArray, queue_size=1)
    #wparray = asv_msgs.msg.WPArray()
    #wparray.wp_xy.append(asv_msgs.msg.WP()) # one WP (i.e. nwp) added, do we need more? e.g. prev WP?
