...
const float DRVO_PI = 3.141592653589793f;
	...
	{ 
		...
		T_HD_ = T_; 		// 30.0, 15.0
		DT_ = 0.5;		// 0.5 (sim/Euler), run rate should be 0.5*10 s 
    		T_stat_ = T_;		// (50.0)
		...
		OBST_FILTER_ON_ = 1; 	// 1=ON, 0=OFF!
		T_LOST_LIMIT_ = 30.0; 	// 30.0 s obstacle no longer relevant after this time
		T_TRACKED_LIMIT_= 30.0; // 60.0 s obstacle still relevant if tracked for so long, choice depends on survival rate
	...
	void Agent::getBestControlOffset(double &u_os_best, double &psi_os_best, double u_d, double psi_d, const Eigen::Matrix<double,6,1>& asv_state, const Eigen::Matrix<double,-1,10>& obst_states, const Eigen::Matrix<double,-1,4>& static_obst, const Eigen::Matrix<double,-1,2>& next_waypoints){
		...
		if (obst_states.rows() == 0 && oldObstacles_.size() == 0 ){
			u_best = u_d; u_os_best = 1;
			psi_best = psi_d; psi_best = 0;
			P_ca_last_ = 1;
			Chi_ca_last_ = 0;
			Chi_ca_last_0 = 0;
			...
			for (int i = 0; i < obst_states.rows(); i++){
				...
				for (int j = 0; j < oldObstacles_.size(); j++){
					...
					if( (double)oldObstacles_[j]->id_ == obst_states(i,9)){
						...
						oldObstacles_[j]->durationLost = 0.0; // update/reset duration lost
			...
			if (OBST_FILTER_ON_){
				for (int j = 0; j < oldObstacles_.size(); j++){
					...
					oldObstacles_[j]->durationLost += DT_*10; // increment is the same as dt_f in obstacle!
					...
					if( oldObstacles_[j]->durationTracked >= T_TRACKED_LIMIT_ &&  
						...
						std::cout << "NB! obstacle " << oldObstacles_[j]->id_ <<  " lost " << oldObstacles_[j]->durationLost << "s ago, but allowed to live!" << std::endl; // check test!
		...
		for (int k = 0; k < n_obst; k++){
			...
			CRG_0(k) = v_s.dot(v_o) < cos(22.5*DEG2RAD)*v_s.norm()*v_o.norm() 
				...
				&& ( (SB_0(k) && v3_x(2) > 0) || (!SB_0(k) && v3_x(2) < 0) ) // < || > (ENU), > || < (NED/MR)  
				&& AH_0(k) && !OBS_OUT_OF_WAY_0(k);
				//&& !OTG_0(k) && !OT_0(k);
				//&& dist_0(k) < D_CLOSE_;
	...
	void Agent::computeDynamicWeights()
	{
		...
		std::cout << "neighbors_.size() : " << neighbors_.size() << std::endl; // check test!
		...
		//for (int k = 0; k < n_obst; k++){
			...
			std::cout << " alpha_l.rows() : "<< alpha_l.rows()  << std::endl;			
			if (alpha_l.rows() > 0 ){
			...
			if ( !OBS_OUT_OF_WAY_0(k) && other->durationLost>10.0 ){
				alpha_0(k) = alpha_0(k) * (2*DT_*10.0/other->durationLost); // reduce the influence of this obstacle 
			}
	...
	{	
		...
		for (std::set< std::pair<float, std::size_t> >::const_iterator iter = neighbors_.begin(); iter != neighbors_.end(); ++iter) { //loop through neighbors
			...
			velocityObstacle.side1_ = Vector2(std::cos(angle - openingAngle), std::sin(angle - openingAngle)); // unit vector of the right (ENU*), (or left NED/MR) side of VO

			velocityObstacle.side2_ = Vector2(std::cos(angle + openingAngle), std::sin(angle + openingAngle)); // unit vector of the left (ENU*), (or right NED/MR) side of VO
			...
			obst_right_passage = det(other->position_ - position_, velocity_ - other->velocity_) > 0.0f; //  (P_B-P_A) x (v_A-v_B) > 0 (ENU), < 0 (NED/MR) 
			obst_right_passage_strict = det(velocityObstacle.side2_, velocity_ - other->velocity_) > 0.0f; // (lambda_r) x (v_A-v_B) > 0 (ENU), < 0 (NED/MR) 
			obst_left_passage_strict = det(velocityObstacle.side1_, velocity_ - other->velocity_) < 0.0f; // (lambda_l) x (v_A-v_B) < 0 (ENU), > 0 (NED/MR)
			
			
			/*
			obst_right_passage = det(velocity_ - other->velocity_, other->position_ - position_) > 0.0f; //  (P_B-P_A) x (v_A-v_B) > 0 (ENU), < 0 (NED/MR) 
			obst_right_passage_strict = det(velocity_ - other->velocity_, velocityObstacle.side2_) > 0.0f; // (lambda_r) x (v_A-v_B) > 0 (ENU), < 0 (NED/MR) 
			obst_left_passage_strict = det(velocity_ - other->velocity_, velocityObstacle.side1_) < 0.0f; // (lambda_l) x (v_A-v_B) < 0 (ENU), > 0 (NED/MR)
			*/
			...
			//if (obst_right_passage && !CRG && !HO && (OTG || OT || !AH ) ) { // 
				...
				const float s = (1.0f - alpha)  * det(velocity_ - other->velocity_, velocityObstacle.side1_) / d; // 1_ is right (ENU*), 2_ is right (NED)

				// compute VO apex
				velocityObstacle.apex_ = other->velocity_ - s * velocityObstacle.side2_; // 2_ is left (ENU), 1_ is left (NED) ; *** working OK, if (-s) is used, else apex remains almost same for all alpha!
				
				// pass this obstacle on the right side
				velocityObstacle.side2_ = -normal(position_, other->position_); // block left: 2_ is left (ENU*), 1_ is left (NED)
				...
				const float s = (1.0f - alpha) * det(velocity_ - other->velocity_, velocityObstacle.side2_) / d; // 2_ is left (ENU*), 1_ is left (NED) 

				// compute VO apex
				velocityObstacle.apex_ = other->velocity_ + s * velocityObstacle.side1_; // 1_ is right (ENU*), 2_ is right (NED)

				// pass this obstacle on the left side
				velocityObstacle.side1_ = normal(position_, other->position_); //block right: 1_ is right (ENU*), 2_ is right (NED)
				//velocityObstacle.side2_ = -velocityObstacle.side2_; // test** s1=R, s2=L: turn L => keep L change R 
 ...
 */
		...
		for (std::multimap<float, Candidate>::const_iterator iter = candidates_.begin(); iter != candidates_.end(); ++iter) {
			...
			for (int j = 0; j < static_cast<int>(velocityObstacles_.size()); ++j) {
				...
				&& det(velocityObstacles_[j].side2_, candidate.position_ - velocityObstacles_[j].apex_) < 0.0f // (<) 2_ is left (ENU*), (>) 1_ is left (NED/MR)
				&& det(velocityObstacles_[j].side1_, candidate.position_ - velocityObstacles_[j].apex_) > 0.0f // (>) 1_ is right (ENU*), (<) 2_ is right (NED/MR)
				) {
					valid = false;					
			...
			for (std::set< std::pair<float, std::size_t> >::const_iterator iter = neighbors_.begin(); iter != neighbors_.end(); ++iter) {
				...
				double SB_t0 = phi0 >= 0; // < 0 (ENU), >= 0 (NED/MR)
				...
				// obstacle at starboard/port side at current CPA
				double SB_tNd = phiNd >= 0; // < 0 (ENU), >= 0 (NED/MR)
				...
				*/	
				...
				if(!valid && ((HO && !SB) || (CRG ) || dist_0(iter->second) < D_CRIT_) && (!OBS_OUT_OF_WAY || (alpha_0(iter->second)>0 && !OBS_OUT_OF_WAY)) ){ // should OBS_OUT_OF_WAY be considered here? Yes, since <!valid> applies to only !OBS_OUT_OF_WAY!
					...
					// if the relative velocity is on the left (ENU*) of the VO center line  
					obst_right_passage = det(other->position_ - position_, velocity_ - other->velocity_) > 0.0f; // > (ENU), < (NED/MR); expected side to pass! (P_B-P_A) x (v_A-v_B) > 0 
				...
				int conv = -1; // NED/MR: -1, ENU: 1 , OBS! not tested
				...
				*/
				...
				p1 << asv->y[n_geo_samp-1],asv->x[n_geo_samp-1];    // End of trajectory
				...
				// check position for collision danger in the vicinity 
	...
	double Agent::getTLostLimit(){
	    	return T_LOST_LIMIT_;
	}
	
	double Agent::getTTrackedLimit(){
	...
	void Agent::setTLostLimit(double tLostLimit){
	    T_LOST_LIMIT_ = tLostLimit;
	}
	
	void Agent::setTTrackedLimit(double tTrackedLimit){
