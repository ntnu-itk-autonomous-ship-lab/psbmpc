...
void KalmanFilter::update(const Eigen::VectorXd& y, double durationLost) {
  ...
  if (durationLost == 0.0){  
	  ...
	  if (m==n){
	      for (int i=0; i<n; i++){
		  if ( angle_tag(i) == 1 ) 
		      y_diff(i) = angle_diff(x_hat_new(i), y(i)); // y - C*x_hat_new	  
	      } 
	  }
...
void KalmanFilter::update(const Eigen::VectorXd& y, double durationLost, double dt, const Eigen::MatrixXd A) {
