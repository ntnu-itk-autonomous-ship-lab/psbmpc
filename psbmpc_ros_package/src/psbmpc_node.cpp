 ...
 *
 *  Function  : Class functions for the PSBMPC ROS node
 *
 *
 *	           ---------------------
 *
...
PSBMPC_Node::PSBMPC_Node(
	ros::NodeHandle &nh // In: Handle to the PSBMPC node, initialized in the main loop
	) : nh(nh), synchronizer(ownship_pose_subscriber, ownship_velocity_subscriber, 1)
{
	...
	supervisor_mode_topic_name = nh.param<std::string>("supervisor_mode_topic_name", "");
	psbmpc_mode_topic_name = nh.param<std::string>("psbmpc_mode_topic_name", "");

	set_psbmpc_mode_service_name = nh.param<std::string>("set_psbmpc_mode_service_name", "");
	waypoint_loader_service_name = nh.param<std::string>("waypoint_loader_service_name", "");
	...
	trajectory_downsampling_factor = nh.param<double>("trajectory_downsampling_factor", 0.5);
	...
	chi_offsets = nh.param<std::string>("sbmpc_parameters/chi_offsets", chi_offsets);
	...
	chi_offsets = nh.param<std::string>("psbmpc_parameters/chi_offsets", chi_offsets);
	...
	ownship = PSBMPC_LIB::CPU::Ownship(ownship_length, w, T_chi, T_U, R_a, LOS_LD, LOS_K_i);
	sbmpc = PSBMPC_LIB::SBMPC(ownship, sbmpc_pars);

#if USE_GPU_PSBMPC
	PSBMPC_LIB::CVE_Pars<float> cve_pars;
	cve_pars.d_init_colregs_situation = nh.param<float>("cve/d_init_colregs_situation", cve_pars.d_init_colregs_situation);
	cve_pars.head_on_width = nh.param<float>("cve/head_on_width", cve_pars.head_on_width) * DEG2RAD;
	cve_pars.overtaking_angle = nh.param<float>("cve/overtaking_angle", cve_pars.overtaking_angle) * DEG2RAD;
	cve_pars.critical_distance_to_ignore_SO = nh.param<float>("cve/critical_distance_to_ignore_SO", cve_pars.critical_distance_to_ignore_SO);
	cve_pars.max_acceptable_SO_speed_change = nh.param<float>("cve/max_acceptable_SO_speed_change", cve_pars.max_acceptable_SO_speed_change);
	cve_pars.max_acceptable_SO_course_change = nh.param<float>("cve/max_acceptable_SO_course_change", cve_pars.max_acceptable_SO_course_change) * DEG2RAD;
	cve_pars.min_acceptable_GW_speed_change = nh.param<float>("cve/min_acceptable_GW_speed_change", cve_pars.min_acceptable_GW_speed_change);
	cve_pars.min_acceptable_GW_course_change = nh.param<float>("cve/min_acceptable_GW_course_change", cve_pars.min_acceptable_GW_course_change) * DEG2RAD;
	cve_pars.GW_safety_margin = nh.param<float>("cve/GW_safety_margin", cve_pars.GW_safety_margin);
	PSBMPC_LIB::GPU::Ownship ownship_gpu(ownship_length, w, T_chi, T_U, R_a, LOS_LD, LOS_K_i);
	psbmpc = PSBMPC_LIB::GPU::PSBMPC(ownship_gpu, cpe, psbmpc_pars, cve_pars);
#else
	/* PSBMPC_LIB::CVE_Pars<double> cve_pars;
	cve_pars.max_distance_at_cpa = nh.param<double>("cve/max_distance_at_cpa", cve_pars.max_distance_at_cpa);
	cve_pars.d_init_colregs_situation = nh.param<double>("cve/d_init_colregs_situation", cve_pars.d_init_colregs_situation);
	cve_pars.head_on_width = nh.param<double>("cve/head_on_width", cve_pars.head_on_width);
	cve_pars.overtaking_angle = nh.param<double>("cve/overtaking_angle", cve_pars.overtaking_angle);
	cve_pars.max_acceptable_SO_speed_change = nh.param<double>("cve/max_acceptable_SO_speed_change", cve_pars.max_acceptable_SO_speed_change);
	cve_pars.max_acceptable_SO_course_change = nh.param<double>("cve/max_acceptable_SO_course_change", cve_pars.max_acceptable_SO_course_change);
	cve_pars.critical_distance_to_ignore_SO = nh.param<double>("cve/critical_distance_to_ignore_SO", cve_pars.critical_distance_to_ignore_SO);
	cve_pars.GW_safety_margin = nh.param<double>("cve/max_acceptable_SO_course_change", cve_pars.GW_safety_margin); */
	...
	//=========================================================================
	psbmpc_mode_publisher = nh.advertise<std_msgs::String>(psbmpc_mode_topic_name, 1);
	psbmpc_mode_publish_timer = nh.createTimer(
		ros::Duration(1.0 / psbmpc_mode_publish_rate),
		std::bind(&PSBMPC_Node::publish_psbmpc_mode, this));

	trajectory_publisher = nh.advertise<custom_msgs::Trajectory4>(trajectory_reference_topic_name, 1);
	...
	polygons_subscriber = nh.subscribe(polygons_topic_name, 1, &PSBMPC_Node::polygons_callback, this);

	do_data_subscriber = nh.subscribe(do_data_topic_name, 1, &PSBMPC_Node::do_data_callback, this);
	...
	synchronizer.registerCallback(boost::bind(&PSBMPC_Node::ownship_state_callback, this, _1, _2));

	waypoint_subscriber = nh.subscribe(waypoint_topic_name, 1, &PSBMPC_Node::waypoint_callback, this);

	wind_subscriber = nh.subscribe(wind_topic_name, 1, &PSBMPC_Node::wind_callback, this);

	supervisor_mode_subscriber = nh.subscribe(supervisor_mode_topic_name, 1, &PSBMPC_Node::supervisor_mode_callback, this);

	set_psbmpc_mode_service = nh.advertiseService(set_psbmpc_mode_service_name, &PSBMPC_Node::set_psbmpc_mode_handler, this);

	waypoint_loader_service = nh.advertiseService(waypoint_loader_service_name, &PSBMPC_Node::load_waypoints_handler, this);
}

/****************************************************************************************
 *  Name     : publish_psbmpc_mode
 *  Function : Runs the PSB-MPC algorithm on the current information and publishes the
 *			  optimal solution.
 *  Method   :
 *  Author   :
 *****************************************************************************************/
...
{
	std::scoped_lock psbmpc_data_lock(
		ownship_data_mutex,
		obstacles_data_mutex,
		polygons_data_mutex,
		wp_data_mutex,
		wind_data_mutex,
		supervisor_mode_data_mutex,
	...
	std::cout << "U_d = " << U_d[ownship.get_wp_counter()] << " | chi_d = " << chi_d << std::endl;
	for (size_t i = 0; i < obstacles.size(); i++)
	{
		std::cout << "DO " << i << ": state = " << obstacles[i].kf.get_state().transpose() << std::endl;
		std::cout << "DO " << i << ": Pr_WGW = " << obstacles[i].get_Pr_WGW() << "Pr_CCEM = " << obstacles[i].get_Pr_CCEM() << std::endl;
		std::cout << "DO " << i << ": Pr_s_i = " << obstacles[i].get_scenario_probabilities().transpose() << std::endl;
	} */
	...
	for (int k = 0; k < n_samples; k += trajectory_downsampling_factor)
	{
		...
		trajectory_msg.chi_p.push_back(PSBMPC_LIB::CPU::wrap_angle_to_pmpi(predicted_trajectory(2, k)));
#if (OWNSHIP_TYPE == 0)
		trajectory_msg.U_p.push_back(predicted_trajectory(3, k));
#else
		trajectory_msg.U_p.push_back(sqrt(pow(predicted_trajectory(3, k), 2) + pow(predicted_trajectory(4, k), 2)));
#endif
	}
 ...
 *  Function : Collects dynamic obstacle data: ID, dims, state, predicted trajectories.
 *		      For all obstacles. Includes intent info if possible.
 *  Method   :
 *  Author   :
 *****************************************************************************************/
void PSBMPC_Node::do_data_callback(
	const ros_af_msgs::DynamicObstaclesData::ConstPtr &msg // In: ROS message containing obstacle data
)
{
	...
	for (int i = 0; i < n_do; i++)
	{
		...
		dims << msg->do_data[i].dimensions.x,
			msg->do_data[i].dimensions.y,
			msg->do_data[i].dimensions.z,
			msg->do_data[i].dimensions.w;
		...
		for (int ps = 0; ps < n_ps_i; ps++)
		{
			...
			{
				xs_i_ps_k(0) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].pos_est.x;
				xs_i_ps_k(1) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].pos_est.y;
				xs_i_ps_k(2) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].vel_est.x;
				xs_i_ps_k(3) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].vel_est.y;
				xs_i_p[ps].col(k) = xs_i_ps_k;

				if (ps == 0)
				{
					P(0, 0) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].pos_cov.var_x;
					P(0, 1) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].pos_cov.corr_xy;
					P(1, 0) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].pos_cov.corr_xy;
					P(1, 1) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].pos_cov.var_y;

					P(2, 2) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].vel_cov.var_x;
					P(0, 1) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].vel_cov.corr_xy;
					P(1, 0) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].vel_cov.corr_xy;
					P(3, 3) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].vel_cov.var_y;

					pv_corr(0, 0) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].pos_vel_corr.corr_px_vx;
					pv_corr(0, 1) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].pos_vel_corr.corr_px_vy;
					pv_corr(1, 0) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].pos_vel_corr.corr_py_vx;
					pv_corr(1, 1) = msg->do_data[i].predicted_trajectories[ps].trajectory[k].pos_vel_corr.corr_py_vy;
		...
		// std::cout << "PN i = " << i << ": Pr_WGW = " << Pr_WGW << " | Pr_CCEM = " << Pr_CCEM << std::endl;
		to = PSBMPC_LIB::Tracked_Obstacle(xs_i_p, P_i_p, Pr_s_i, Pr_WGW, Pr_CCEM, dims, duration_tracked, duration_lost, ID);
...
)
{
	...
	if (waypoints.cols() > 1)
	{
		...
		ownship.update_guidance_references(U_d[ownship.get_wp_counter()], chi_d, waypoints, ownship_state, 0.0, PSBMPC_LIB::LOS);
	}
}
...
void PSBMPC_Node::wind_callback(
	const geometry_msgs::TwistStamped::ConstPtr &msg // In: ROS message containing wind information
)
{
...
bool PSBMPC_Node::load_waypoints_handler(
	psbmpc::LoadWaypoints::Request &req, // In: Request information for loading waypoints
	psbmpc::LoadWaypoints::Response &res // Out: Response information after loading waypoints
)
{
	try
	{
		...
		std::string absolute_filename = ros::package::getPath("psbmpc") + "/waypoint_data/" + req.waypoints_yaml_file;
...
/****************************************************************************************
 *  Name     : main
 *  Function : Sets up, initializes, runs the PSBMPC ROS node, and shuts it down.
 *  Method   :
 *  Author   :
 *****************************************************************************************/
