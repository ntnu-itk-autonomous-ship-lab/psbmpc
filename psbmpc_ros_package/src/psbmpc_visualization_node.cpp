...
PSBMPC_Visualization_Node::PSBMPC_Visualization_Node(
    ros::NodeHandle &
        nh // In: Handle to the visualization node, initialized in the main loop
    )
    : synchronizer(ownship_pose_subscriber, ownship_velocity_subscriber, 1),
      nh(nh) {
  trajectory_reference_vis_topic_name =
      nh.param<std::string>("trajectory_reference_vis_topic_name", "");
  trajectory_vis_topic_name =
      nh.param<std::string>("trajectory_vis_topic_name", "");
  ...
  polygons_vis_topic_name =
      nh.param<std::string>("polygons_vis_topic_name", "");
  ...
  do_trajectories_vis_topic_name =
      nh.param<std::string>("do_trajectories_vis_topic_name", "");

  get_polygons_service_name =
      nh.param<std::string>("get_polygons_service_name", "");

  trajectory_reference_vis_publish_rate =
      nh.param<double>("trajectory_reference_vis_publish_rate", 0.5);
  polygons_vis_publish_rate =
      nh.param<double>("polygons_vis_publish_rate", 0.1);

  d_so_relevant_viz = nh.param<double>("d_so_relevant_viz", 200.0);

  trajectory_reference_scale_x =
      nh.param<double>("trajectory_reference/scale_x", 0.2);
  trajectory_reference_color =
      nh.param<std::vector<double>>("trajectory_reference/color", {});
  trajectory_update_threshold =
      nh.param<double>("trajectory/update_threshold", 0.2);
  ...
  prediction_method =
      nh.param<int>("/psbmpc/psbmpc_parameters/prediction_method", 100);
  guidance_method =
      nh.param<int>("/psbmpc/psbmpc_parameters/guidance_method", 100);
  ...
  //=========================================================================
  trajectory_reference_vis_publisher =
      nh.advertise<visualization_msgs::MarkerArray>(
          trajectory_reference_vis_topic_name, 10);
  trajectory_reference_vis_publish_timer =
      nh.createTimer(ros::Duration(1.0 / trajectory_reference_vis_publish_rate),
                     std::bind(&PSBMPC_Visualization_Node::
                                   publish_trajectory_reference_visualization,
                               this));

  trajectory_vis_publisher =
      nh.advertise<visualization_msgs::Marker>(trajectory_vis_topic_name, 10);

  pose_vis_publisher =
      nh.advertise<visualization_msgs::Marker>(pose_vis_topic_name, 10);

  polygons_vis_publisher = nh.advertise<visualization_msgs::MarkerArray>(
      polygons_vis_topic_name, 10);
  polygons_vis_publish_timer = nh.createTimer(
      ros::Duration(1.0 / polygons_vis_publish_rate),
      std::bind(&PSBMPC_Visualization_Node::publish_polygons_visualization,
                this));

  do_trajectories_vis_publisher = nh.advertise<visualization_msgs::MarkerArray>(
      do_trajectories_vis_topic_name, 10);
  do_trajectories_subscriber =
      nh.subscribe(do_data_topic_name, 1,
                   &PSBMPC_Visualization_Node::publish_do_trajectories, this);
  ...
  synchronizer.registerCallback(boost::bind(
      &PSBMPC_Visualization_Node::ownship_state_callback, this, _1, _2));

  trajectory_reference_subscriber = nh.subscribe(
      trajectory_reference_topic_name, 1,
      &PSBMPC_Visualization_Node::trajectory_reference_callback, this);

  waypoint_subscriber =
      nh.subscribe(waypoint_topic_name, 10,
                   &PSBMPC_Visualization_Node::waypoint_callback, this);

  get_polygons_client =
      nh.serviceClient<psbmpc::GetPolygons>(get_polygons_service_name);
 ...
 *  Function : Publishes a visualization marker for  the current predicted
 * PSBMPC trajectory Method   : Author   :
 *****************************************************************************************/
void PSBMPC_Visualization_Node::publish_trajectory_reference_visualization() {
  std::scoped_lock trajectory_reference_data_lock(
      trajectory_reference_data_mutex);
  ...
  // Create marker for the trajectory reference and the safety_zone at the
  // current time
  visualization_msgs::Marker trajectory_reference, safety_zone, wps;
  trajectory_reference.header.frame_id = safety_zone.header.frame_id =
      wps.header.frame_id = nh.param<std::string>("/frames/local_NED", "");
  trajectory_reference.header.stamp = safety_zone.header.stamp =
      wps.header.stamp = ros::Time::now();
  trajectory_reference.ns = safety_zone.ns = wps.ns =
      "psbmpc_trajectory_visualization";
  trajectory_reference.action = safety_zone.action = wps.action =
      visualization_msgs::Marker::ADD;
  trajectory_reference.pose.orientation.w = safety_zone.pose.orientation.w =
...
void PSBMPC_Visualization_Node::publish_do_trajectories(
    const ros_af_msgs::DynamicObstaclesData::ConstPtr
        &msg // In: Message containing dynamic obstacle data
) {
  ...
  trajectory_vis.header.frame_id =
      nh.param<std::string>("/frames/local_NED", "");
  ...
  for (const auto &ship_msg : msg->do_data) {
    ...
    // std::cout << "DO " << ship_msg.ID << " | trajectory probs = " <<
...
void PSBMPC_Visualization_Node::trajectory_reference_callback(
    const custom_msgs::Trajectory4::ConstPtr
        &msg // In: Message containing PSBMPC predicted trajectory
) {
  std::scoped_lock trajectory_reference_data_lock(
      trajectory_reference_data_mutex);
...
void PSBMPC_Visualization_Node::ownship_state_callback(
  ...
  t_now_trajectory_update =
      (double)ros::Time::now().sec + (double)ros::Time::now().nsec * 1e-9;
  ...
  double dt_trajectory_update =
      t_now_trajectory_update - t_prev_trajectory_update;
...
void PSBMPC_Visualization_Node::relevant_psbmpc_polygons_callback(
  ...
  std::scoped_lock simplified_polygons_data_lock(
      simplified_polygons_data_mutex);
...
/****************************************************************************************
 *  Name     : main
 *  Function : Sets up, initializes, runs the visualization node, and shuts it
 * down. Method   : Author   :
 *****************************************************************************************/
