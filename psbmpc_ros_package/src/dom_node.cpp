 ...
 *  Author   :
 *****************************************************************************************/
DOM_Node::DOM_Node(ros::NodeHandle &nh // In: Handle to the DOM node,
  ...
  T_tracked_limit =
      nh.param<double>("manager/T_tracked_limit", T_tracked_limit);
  obstacle_filter_on =
      nh.param<bool>("manager/obstacle_filter_on", obstacle_filter_on);
  psbmpc_do_manager = PSBMPC_LIB::Obstacle_Manager(
      T_lost_limit, T_tracked_limit, obstacle_filter_on);
  sbmpc_do_manager = PSBMPC_LIB::Obstacle_Manager(T_lost_limit, T_tracked_limit,
                                                  obstacle_filter_on);

  double r_ct(10.0), sigma_x(0.01), sigma_xy(0.0), sigma_y(0.01), gamma_x(0.1),
      gamma_y(0.1);
  ...
  sbmpc_do_predictor = PSBMPC_LIB::Obstacle_Predictor(
      r_ct, sigma_x, sigma_xy, sigma_y, gamma_x, gamma_y, sbmpc_pars);

  psbmpc_do_predictor = PSBMPC_LIB::Obstacle_Predictor(
      r_ct, sigma_x, sigma_xy, sigma_y, gamma_x, gamma_y, psbmpc_pars);
  ...
  //=========================================================================
  do_data_publisher =
      nh.advertise<ros_af_msgs::DynamicObstaclesData>(do_data_topic_name, 1);
  do_data_publish_timer =
      nh.createTimer(ros::Duration(1.0 / do_data_publish_rate),
                     std::bind(&DOM_Node::publish_do_data, this));

  do_estimate_subscriber = nh.subscribe(do_estimate_topic_name, 1,
                                        &DOM_Node::do_estimate_callback, this);

  polygons_subscriber =
      nh.subscribe(polygons_topic_name, 1, &DOM_Node::polygons_callback, this);
  ...
  synchronizer.registerCallback(
      boost::bind(&DOM_Node::ownship_state_callback, this, _1, _2));

  psbmpc_mode_subscriber = nh.subscribe(psbmpc_mode_topic_name, 1,
                                        &DOM_Node::psbmpc_mode_callback, this);
...
void DOM_Node::publish_do_data() {
  std::scoped_lock dom_data_lock(ownship_data_mutex, do_estimate_data_mutex,
                                 track_data_mutex, polygons_data_mutex);

  bool initialized = ownship_state.size() >= 4 && obstacle_states.cols() > 0 &&
                     obstacle_covariances.cols() > 0;
  if (!initialized) {
    return;
  }
  ...
  if (psbmpc_mode == "sbmpc") {
    sbmpc_do_manager(obstacle_states, obstacle_covariances, sbmpc_pars,
                     t_now - t_prev);

    sbmpc_do_predictor(sbmpc_do_manager.get_data(), ownship_state, sbmpc_pars,
                       true);

    obstacles = sbmpc_do_manager.get_data();
    dt_p = sbmpc_pars.get_dpar(i_dpar_dt_SBMPC);
  } else if (psbmpc_mode == "psbmpc") {
    psbmpc_do_manager(obstacle_states, obstacle_covariances, psbmpc_pars,
                      t_now - t_prev);

    psbmpc_do_predictor.operator()(psbmpc_do_manager.get_data(), ownship_state,
                                   psbmpc_pars);
  ...
  for (int i = 0; i < n_do; i++) {
    do_data_msg.do_data[i].header.frame_id =
    ...
    d_0i =
        (xs_i_p[0].block<2, 1>(0, 0) - ownship_state.block<2, 1>(0, 0)).norm() -
        l;
  ...
  // ROS_WARN("Publishing %d trajectories",
  // (int)do_data_msg.do_data[0].predicted_trajectories.size());
 ...
 *  Function : Collects dynamic obstacle state information for the ownship. for
 * all observable obstacles. Method   : Author   :
 *****************************************************************************************/
void DOM_Node::do_estimate_callback(
    const ros_af_msgs::DynamicObstacleEstimate::ConstPtr
        &msg // In: ROS message containing a dynamic obstacle ID, size estimate,
  ...
  while (ID >= obstacle_states.cols()) {
    ...
    obstacle_covariances.conservativeResize(16,
                                            obstacle_covariances.cols() + 1);
    do_time_stamps.conservativeResize(do_time_stamps.size() + 1);
  }

  double A(msg->dimensions.x), B(msg->dimensions.y), C(msg->dimensions.z),
      D(msg->dimensions.w);
  Eigen::Vector4d xs_i;
  Eigen::Matrix4d P;
  ...
  pos_vel_cov(0, 0) =
      msg->kinematic_estimate.kinematic_estimate.pos_vel_corr.corr_px_vx;
  pos_vel_cov(0, 1) =
      msg->kinematic_estimate.kinematic_estimate.pos_vel_corr.corr_px_vy;
  pos_vel_cov(1, 0) =
      msg->kinematic_estimate.kinematic_estimate.pos_vel_corr.corr_py_vx;
  pos_vel_cov(1, 1) =
      msg->kinematic_estimate.kinematic_estimate.pos_vel_corr.corr_py_vy;
