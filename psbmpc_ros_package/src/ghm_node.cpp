/****************************************************************************************
 *
 *  File name : ghm_node.cpp
 *
 *  Function  : Class functions for the PSBMPC Grounding Hazard Manager ROS node
 *
 *	           ---------------------
 *
 *  Version 1.0
 ...
 *****************************************************************************************/
GHM_Node::GHM_Node(
	ros::NodeHandle &nh // In: Handle to the GHM node, initialized in the main loop
	) : nh(nh), synchronizer(ownship_pose_subscriber, ownship_velocity_subscriber, 1)
{
	...
	int utm_zone(1);
	map_data_filename = nh.param<std::string>("map_data_filename", map_data_filename);
	equatorial_radius = nh.param<double>("coordinate_frame_parameters/equatorial_radius", 0.0);
	flattening_factor = nh.param<double>("coordinate_frame_parameters/flattening_factor", 0.0);
	utm_zone = nh.param<int>("coordinate_frame_parameters/utm_zone", 32);
	local_ned_frame_name = nh.param<std::string>("coordinate_frame_parameters/reference_frame/name", "");
	local_ned_frame_name = nh.param<std::string>(local_ned_frame_name, "");
	local_ned_frame_lla_parameter_name = nh.param<std::string>("coordinate_frame_parameters/reference_frame/lla", "");
	frame_origin = nh.param<std::vector<double>>(local_ned_frame_lla_parameter_name, frame_origin);
	...
	grounding_hazard_manager.read_other_polygons(package_path + "/" + map_data_filename, true, false);
	polygons_ned = grounding_hazard_manager.get_polygons_ned();
	simplified_polygons_ned = grounding_hazard_manager.get_simplified_polygons_ned();

	//=========================================================================
	// Initialize clients, services, publishers and subscribers
	//=========================================================================
	polygons_publisher = nh.advertise<custom_msgs::Polygons>(polygons_topic_name, 1);
	polygons_publish_timer = nh.createTimer(
		ros::Duration(1.0 / polygons_publish_rate),
	...
	synchronizer.registerCallback(boost::bind(&GHM_Node::ownship_state_callback, this, _1, _2));

	psbmpc_mode_subscriber = nh.subscribe(psbmpc_mode_topic_name, 1, &GHM_Node::psbmpc_mode_callback, this);

	get_polygons_server = nh.advertiseService(get_polygons_service_name, &GHM_Node::get_polygons_handler, this);
}
...
void GHM_Node::publish_polygons()
{
	...
	std::scoped_lock ghm_data_lock(ownship_data_mutex, polygon_data_mutex, psbmpc_mode_data_mutex);

	if (psbmpc_mode == "sbmpc")
	{
		relevant_polygons_ned = grounding_hazard_manager(ownship_state, sbmpc_pars.get_dpar(i_dpar_d_so_relevant_SBMPC));
	}
	else if (psbmpc_mode == "psbmpc")
	{
		relevant_polygons_ned = grounding_hazard_manager(ownship_state, psbmpc_pars.get_dpar(i_dpar_d_so_relevant));
	}
	...
	for (int j = 0; j < n_polygons; j++)
	{
		...
		for (auto it = boost::begin(boost::geometry::exterior_ring(relevant_polygons_ned[j])); it != boost::end(boost::geometry::exterior_ring(relevant_polygons_ned[j])); it++)
		{
...
bool GHM_Node::get_polygons_handler(
	psbmpc::GetPolygons::Request &req, // In: Request information for getting polygons (empty)
	psbmpc::GetPolygons::Response &res // Out: Response information after sending polygons
)
{
	try
	{
		std::scoped_lock polygon_server_data_lock(ownship_data_mutex, polygon_data_mutex);
		...
		PSBMPC_LIB::Static_Obstacles response_polygons = grounding_hazard_manager(ownship_state, d_so_relevant);
		...
		{
			geometry_msgs::Polygon polygon;
			for (auto it = boost::begin(boost::geometry::exterior_ring(response_polygons[j])); it != boost::end(boost::geometry::exterior_ring(response_polygons[j])); it++)
			{
				point.x = boost::geometry::get<0>(*it);
				point.y = boost::geometry::get<1>(*it);
				point.z = 0.0;
