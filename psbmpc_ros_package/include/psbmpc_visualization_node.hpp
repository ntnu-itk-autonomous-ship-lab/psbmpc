...
private:
  ...
  double trajectory_vis_publish_rate, trajectory_reference_vis_publish_rate,
  ...
  double trajectory_scale_x, trajectory_reference_scale_x, safety_zone_scale_x,
      polygon_scale_x;
  std::vector<double> trajectory_color, trajectory_reference_color,
      safety_zone_color, pose_color, pose_scale, wps_color, wps_scale,
      polygon_color;

  PSBMPC_LIB::PSBMPC_Parameters psbmpc_pars;

  std::mutex trajectory_reference_data_mutex, do_trajectory_data_mutex,
      polygons_data_mutex, simplified_polygons_data_mutex, wp_data_mutex,
      visibility_data_mutex, ownship_data_mutex;

  double t_now_trajectory_update, t_prev_trajectory_update,
...
public:
  ...
  void publish_do_trajectories(
      const ros_af_msgs::DynamicObstaclesData::ConstPtr &msg);

  void
  trajectory_reference_callback(const custom_msgs::Trajectory4::ConstPtr &msg);
  ...
  void
  visualization_polygons_callback(const custom_msgs::Polygons::ConstPtr &msg);

  void
  relevant_psbmpc_polygons_callback(const custom_msgs::Polygons::ConstPtr &msg);
