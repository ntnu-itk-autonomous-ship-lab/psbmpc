...
int main() {
  ...
  P_0 << 25, 0, 0, 0, 0, 25, 0, 0, 0, 0, 0.025, 0, 0, 0, 0, 0.025;
  ...
  for (int i = 0; i < n_do; i++) {
    ...
    if (i == 1) {
      ...
      waypoints_i[i] << xs_i_0[i](0), -600, xs_i_0[i](1), 0;
      u_d_i[i] = 4.0;
      chi_d_i[i] = 0 * DEG2RAD;
    }
  ...
  std::vector<polygon_2D> polygons(0);
  double V_w = 0.0;
  ...
  mxArray *T_sim_mx(nullptr), *dt_sim_mx(nullptr), *n_do_mx(nullptr);
  ...
  mxArray *d_safe_mx =
      mxCreateDoubleScalar(psbmpc.pars.get_dpar(i_dpar_d_safe));
  ...
  for (int k = 0; k < N; k++) {
    ...
    for (int i = 0; i < n_do; i++) {
      ...
      Eigen::Map<Eigen::MatrixXd> map_traj_i(ptraj_i, trajectory_i[i].rows(),
                                             N);
