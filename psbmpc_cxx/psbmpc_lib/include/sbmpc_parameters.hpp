...
private:
  ...
  // Number of control behaviours and sequential maneuvers for the ownship,
  // respectively
...
public:
  SBMPC_Parameters() { initialize_pars(false); }

  SBMPC_Parameters(const bool is_obstacle_sbmpc) {
    initialize_pars(is_obstacle_sbmpc);
  }
  SBMPC_Parameters(const std::vector<std::vector<double>> &u_offsets,
                   const std::vector<std::vector<double>> &chi_offsets,
                   const Prediction_Method prediction_method,
                   const Guidance_Method guidance_method,
                   const std::vector<int> &ipars,
                   const std::vector<double> &dpars) {
    initialize_pars(u_offsets, chi_offsets, prediction_method, guidance_method,
                    ipars, dpars);
  }
