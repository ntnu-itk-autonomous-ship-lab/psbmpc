...
// See "Risk-based Maritime Autonomous Collision Avoidance Considering Obstacle
// Intentions" and/or "Collision Probability Estimation for Maritime Collision
// Avoidance Using the Cross-Entropy Method" for more information on CPE
enum CPE_Method {
  CE,     // Consider positional uncertainty only
  MCSKF4D // Consider uncertainty in both position and velocity along piece-wise
          // linear segments
};
...
template <class T> struct CVE_Pars {
  T d_init_colregs_situation; // Threshold for initializing the COLREGS
                              // violation evaluator
  T head_on_width;    // Width of zone where vessels are considered head-on wrt
                      // own-ship
  T overtaking_angle; // Bearing angle wrt own-ship/obstacle where vessels are
                      // considered to overtake
  T critical_distance_to_ignore_SO;  // Distance where the own-ship SO role
                                     // should be aborted to make a safety
                                     // maneuver
  T max_acceptable_SO_speed_change;  // Max allowable stand-on speed change
                                     // before considered violating COLREGS
  T max_acceptable_SO_course_change; // Max allowable stand-on course change
                                     // before considered violating COLREGS
  T min_acceptable_GW_speed_change;  // Min allowable give-way speed change
                                     // before considered violating COLREGS
  T min_acceptable_GW_course_change; // Min allowable give-way course change
                                     // before considered violating COLREGS
  T GW_safety_margin; // Min distance to keep to other vessel for a COLREGS
                      // compliant maneuver to be correct
};

enum COLREGS_Situation { HO, OT_ing, OT_en, CR_PS, CR_SS };

enum class SpeedChange { Lower, None, Higher };

enum class CourseChange { Portwards, None, Starboardwards };
...
private:
  ...
  friend struct GPU::CB_Functor_Pars;
  ...
  // Step between samples in collision probability estimation and
  // grounding cost evaluation, respectively
  int p_step_do, p_step_grounding;

  // Finite sets of offsets considered to the own-ship surge and course
  // references, for each maneuver in the horizon
  ...
  bool use_intention_model, use_path_pruning_ownship,
      use_path_pruning_targetship, use_GPU;

  void initialize_par_limits();

  void initialize_pars();

  void initialize_pars(const std::vector<std::vector<double>> &u_offsets,
...
public:
  ...
  PSBMPC_Parameters(const std::vector<std::vector<double>> &u_offsets,
                    const std::vector<std::vector<double>> &chi_offsets,
                    const CPE_Method cpe_method,
                    const Prediction_Method prediction_method,
                    const Guidance_Method guidance_method,
                    const std::vector<int> &ipars,
                    const std::vector<double> &dpars) {
    initialize_pars(u_offsets, chi_offsets, cpe_method, prediction_method,
                    guidance_method, ipars, dpars);
  ...
  inline Prediction_Method get_prediction_method() const {
    return prediction_method;
  };

  inline Guidance_Method get_guidance_method() const {
    return guidance_method;
  };
};
} // namespace PSBMPC_LIB
