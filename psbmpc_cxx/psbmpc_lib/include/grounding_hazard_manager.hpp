...
private:
  // Distance threshold for a polygon/static obstacle to be relevant for COLAV,
  // and the ramer-douglas-peucker algorithm distance tolerance threshold
  double d_so_relevant, epsilon;

  Static_Obstacles polygons_lla, polygons_ned, simplified_polygons_ned,
      simplified_polygons_lla;
  ...
  /****************************************************************************************
   *  Name     : read_shapefile
   *  Function : Reads a shapefile into the input vector of polygons, using the
   * convert function to convert SHPObjects to Polygons. Author   : Tom Daniel
   * Grande & Trym Tengesdal Modified :
   *****************************************************************************************/
  template <typename T>
  ...
  void convert(SHPObject *ps_shape, polygon_2D &polygon) {
    ...
    // printf("n_parts = %d | n_vertices = %d | Outer part start, end = (%d,
    // %d)\n", ps_shape->nParts, ps_shape->nVertices, outer_part_start,
    // outer_part_end);

    for (int v = outer_part_start; v < outer_part_end; v++) {
      // want the points on format (northing, easting), and relative to the map
      // origin
      point_2D point;
      boost::geometry::assign_values(point, y[v] - map_origin_ned(0),
                                     x[v] - map_origin_ned(1));
   ...
   *  Function : Transforms polygons referenced to map origin in NED, to
   * latitude longitude coordinates Author   : Trym Tengesdal Modified :
   *****************************************************************************************/
  void create_lla_referenced_polygons() {
    ...
    for (int j = 0; j < n_polygons; j++) {
      polygon_2D poly_lla;
      for (auto it =
               boost::begin(boost::geometry::exterior_ring(polygons_ned[j]));
           it != boost::end(boost::geometry::exterior_ring(polygons_ned[j]));
           it++) {
  ...
  void create_ned_referenced_polygons() {
    ...
    for (int j = 0; j < n_polygons; j++) {
      polygon_2D poly_ned;
      for (auto it =
               boost::begin(boost::geometry::exterior_ring(polygons_lla[j]));
           it != boost::end(boost::geometry::exterior_ring(polygons_lla[j]));
           it++) {
        ...
        boost::geometry::assign_values(point_ne, p_ne(0) - map_origin_ned(0),
                                       p_ne(1) - map_origin_ned(1));
   ...
   *****************************************************************************************/
  Static_Obstacles
  create_simplified_polygons(const Static_Obstacles &polygons_in) {
    ...
    for (int j = 0; j < n_polygons; j++) {
      vertices_in.clear();
      for (auto it =
               boost::begin(boost::geometry::exterior_ring(polygons_in[j]));
           it != boost::end(boost::geometry::exterior_ring(polygons_in[j])) - 1;
           it++) {
      ...
      // + 1, vertices_in.size(), vertices_out.size());
    }
    return polygons_out;
  }
   ...
   *  Function : Uses the Ramer Douglas Peucker algorithm to reduce vertices in
   * a polygon, conserving shape as best as possible. Author   : Trym Tengesdal
   *  Modified :
   *****************************************************************************************/
  void ramer_douglas_peucker(
      std::vector<point_2D>
          &vertices_out, // In/Out: Vertices of simplified polygon
      const std::vector<point_2D>
          &vertices_in // In: Vertices of polygon to simplify
  ) {
    ...
    for (size_t i = 1; i < end; i++) {
      d2line = distance_to_line_segment(vertices_in[i], vertices_in[0],
                                        vertices_in[end]);
      if (d2line > d_max) {
        index = i;
        d_max = d2line;
    ...
    if (d_max > epsilon) {
      ...
      std::vector<point_2D> first_line(vertices_in.begin(),
                                       vertices_in.begin() + index + 1);
      std::vector<point_2D> last_line(vertices_in.begin() + index,
                                      vertices_in.end());
      ...
      vertices_out.assign(recursive_results_1.begin(),
                          recursive_results_1.end() - 1);
      vertices_out.insert(vertices_out.end(), recursive_results_2.begin(),
                          recursive_results_2.end());
   ...
   *  Modified :
   *****************************************************************************************/
  double distance_to_line_segment(const point_2D &p, const point_2D &q_1,
...
public:
  ...
  Grounding_Hazard_Manager(const std::string &filename)
      : d_so_relevant(1000), epsilon(2.0), frame_name("local_NED"),
        map_origin_ned(
            7042250,
            270250) // Trondheim, just north of ravnkloa, brattora crossing
  {
  ...
  Grounding_Hazard_Manager(const std::string &filename,
                           const Eigen::Vector2d &map_origin_ned,
                           const MPC_Parameters &mpc_pars)
      : d_so_relevant(mpc_pars.d_so_relevant), epsilon(mpc_pars.epsilon_rdp),
        frame_name("local_NED"), map_origin_ned(map_origin_ned) {
    read_shapefile(filename, polygons_ned);
    create_lla_referenced_polygons();
    simplified_polygons_ned = create_simplified_polygons(polygons_ned);
    simplified_polygons_lla = create_simplified_polygons(polygons_lla);
  ...
  template <class MPC_Parameters>
  Grounding_Hazard_Manager(const std::string &filename,
                           const std::vector<double> &map_origin_ned,
                           const MPC_Parameters &mpc_pars)
      : d_so_relevant(mpc_pars.d_so_relevant), epsilon(mpc_pars.epsilon_rdp),
        frame_name("local_NED") {
  ...
  Grounding_Hazard_Manager(
      ...
      const double
          equatorial_radius, // In: Radius  of ellipsoid parametrization for lla
                             // <-> UTM conversions
      const double
          flattening_factor, // In: Flattening factor of ellipsoid
                             // parametrization for lla <-> UTM conversions
      const int utm_zone, // In: UTM Zone to consider (NOTE: Must match the one
                          // used when generating the shapefiles)
      const bool northp,  // In: Boolean determining if it is the northern or
                          // southern hemisphere
      const Eigen::Vector2d
          &map_origin_lla, // In: Origin of map (from shapefile) in latitude,
                           // longitude format
      const std::string frame_name, // In: Name of resulting local NED frame
      const MPC_Parameters
          &mpc_pars // In: Parameters of calling MPC (Either SB-MPC or PSB-MPC)
      )
      : d_so_relevant(mpc_pars.d_so_relevant), epsilon(mpc_pars.epsilon_rdp),
        frame_name(frame_name),
        utm_p(equatorial_radius, flattening_factor, utm_zone, northp) {
    assert(map_origin_ned.size() == 2);
    utm_p.forward(this->map_origin_ned(1), this->map_origin_ned(0),
                  map_origin_lla(0), map_origin_lla(1));
  ...
  Static_Obstacles get_simplified_polygons_ned() const {
    return simplified_polygons_ned;
  }
  Static_Obstacles get_simplified_polygons_lla() const {
    return simplified_polygons_lla;
  }
  ...
  void switch_local_ned_frame(
      ...
      const bool northp,     // In: Boolean determining if the origin is in the
                             // northern or southern hemisphere
      const Eigen::Vector2d
          &map_origin_lla, // In: Origin of frame in latitude, longitude format
      const std::string frame_name // In: Name of resulting local NED frame
  ) {
    utm_p =
        UTM_Projection(equatorial_radius, flattening_factor, utm_zone, northp);
    assert(map_origin_ned.size() == 2);
    utm_p.forward(this->map_origin_ned(1), this->map_origin_ned(0),
                  map_origin_lla(0), map_origin_lla(1));
   ...
   *  Function : Reads other polygons from a file consisting of a matrix (2 x N)
   * of polygon vertices, separated by columns of -1 (tested for .csv type).
   *		      Either in (easting, northing) or (longitude, latitude
   * format). Author   : Trym Tengesdal Modified :
   *****************************************************************************************/
  void read_other_polygons(
      const std::string &filename, // In: Name of .csv file to process
      const bool
          is_ned_frame, // In: Boolean determining type of coordinates in file
      const bool clear_old_polygons // In: Boolean determining if the old
                                    // polygons should be cleared or not
  ) {
    try {
      if (filename == "") {
        return;
      ...
      Eigen::MatrixXd M =
          PSBMPC_LIB::CPU::read_matrix_from_file<Eigen::MatrixXd>(filename);
      assert(M.rows() == 2);
      int n_cols = M.cols();
      ...
      for (int i = 0; i < n_cols; i++) {
        if (M(0, i) < 0 || i == n_cols - 1) {
          ...
          if (is_ned_frame) {
            // want the points on format (northing, easting), and relative to
            // the map origin
            boost::geometry::assign_values(v, M(1, i) - map_origin_ned(0),
                                           M(0, i) - map_origin_ned(1));
          } else {
  ...
  /****************************************************************************************
   *  Name     : operator()
   *  Function : Returns a vector of relevant static obstacles for use by the
   * PSB/SB-MPC, given the current own-ship position. Two overloads, latter one
   * with input parameter determining radius for relevant obstacles. Author   :
   * Trym Tengesdal Modified :
   *****************************************************************************************/
  Static_Obstacles
  operator()(const Eigen::VectorXd
                 &ownship_state // State of the own-ship, either [x, y, psi, u,
                                // v, r]^T or [x, y, chi, U]^T
  ) {
  ...
  Static_Obstacles operator()(
      const Eigen::VectorXd
          &ownship_state, // State of the own-ship, either [x, y, psi, u, v,
                          // r]^T or [x, y, chi, U]^T
