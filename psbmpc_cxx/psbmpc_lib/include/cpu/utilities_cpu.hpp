...
inline void save_matrix_to_file(const std::string &filename,
                                const Eigen_Type &in) {
 ...
 *  Function : Must be a resizeable eigen matrix type. The matrix size must be
 * given in the first row Author   : Modified :
 *****************************************************************************************/
template <class Eigen_Type>
 ...
 *  Function : Makes sure angle difference a_1 - a_2 is within [-pi, pi]
 * interval Author   : Modified :
 *****************************************************************************************/
 ...
 *  Modified :
 *****************************************************************************************/
inline Eigen::Vector2d rotate_vector_2D(const Eigen::Vector2d &v,
                                        const double angle) {
 ...
 *  Function : Rotates three-dimensional vectory v by angle, around specified
 * axis Author   : Modified :
 *****************************************************************************************/
inline Eigen::Vector3d rotate_vector_3D(const Eigen::Vector3d &v,
                                        const double angle, const Axis axis) {
 ...
 *  Modified :
 *****************************************************************************************/
inline Eigen::Matrix2d rotate_matrix_2D(const Eigen::Matrix2d &M,
                                        const double angle) {
  Eigen::Matrix2d R;
 ...
 *  Modified :
 *****************************************************************************************/
inline Eigen::MatrixXd reshape(const Eigen::VectorXd &in, const int n_rows,
                               const int n_cols) {
  Eigen::MatrixXd out;
  out.resize(n_rows, n_cols);
  int count = 0;
...
/****************************************************************************************
 *  Name     : calculate_cpa
 *  Function : Calculates time, distance (vector) and position (vector) at the
 * Closest Point of Approach between vessel A and B Author   : Modified :
 *****************************************************************************************/
inline void
calculate_cpa(Eigen::Vector2d &p_cpa, // In/out: Position of vessel A at CPA
  ...
  double epsilon =
      0.25; // lower boundary on relative speed to calculate t_cpa "safely"
  ...
  // Check if the relative speed is too low, or if the vessels are moving away
  // from each other with the current velocity vectors
  double dt_incr = 0.1;
  if ((v_A - v_B).norm() < epsilon ||
      (p_A - p_B).norm() <
          ((p_A + v_A * dt_incr) - (p_B + v_B * dt_incr)).norm()) {
    t_cpa = 0;
    p_cpa = p_A;
    d_cpa = (p_A - p_B).norm();
  } else {
 ...
 *  Modified :
 *****************************************************************************************/
inline bool ship_is_passed_by(const Eigen::VectorXd &xs_0,
  ...
  bool A_is_overtaken =
      v_0.dot(v_i) > cos(68.5 * DEG2RAD) * v_0.norm() * v_i.norm() &&
      v_0.norm() < v_i.norm() && v_0.norm() > 0.25;

  bool B_is_overtaken =
      v_i.dot(v_0) > cos(68.5 * DEG2RAD) * v_i.norm() * v_0.norm() &&
      v_i.norm() < v_0.norm() && v_i.norm() > 0.25;

  bool is_passed =
      ((v_0.dot(L_0i) <
            cos(112.5 * DEG2RAD) * v_0.norm() && // Vessel A's perspective
        !A_is_overtaken) ||
       (v_i.dot(-L_0i) <
            cos(112.5 * DEG2RAD) * v_i.norm() && // Vessel B's perspective
        !B_is_overtaken)) &&
      d_0i > d_safe;
  return is_passed;
}
 ...
 *****************************************************************************************/
inline double
evaluateDistance(const Eigen::Vector4d &xs_A, // In: State of vessel A
 ...
 *  Function : Calculates relative bearing of state xs = [x, y, COG, SOG]
 * relative to point (x, y). Author   : Modified :
 *****************************************************************************************/
inline double relativeBearing(const Eigen::Vector4d &xs, const double x,
                              const double y) {
 ...
 *  Modified :
 *****************************************************************************************/
inline Eigen::Vector2d intersectionpoint(const Eigen::Vector4d &line1,
  ...
  res(0) = (sline1.c * sline2.b - sline1.b * sline2.c) /
           (sline1.a * sline2.b - sline1.b * sline2.a);
  res(1) = (sline1.a * sline2.c - sline1.c * sline2.a) /
           (sline1.a * sline2.b - sline1.b * sline2.a);
...
vx_vy_to_heading_speed_state(const Eigen::Vector4d &vx_vy_state) {
  Eigen::Vector4d heading_speed_state = vx_vy_state;
  heading_speed_state(COG) = atan2(vx_vy_state(VY), vx_vy_state(VX));
  heading_speed_state(SOG) =
      sqrtf(powf(vx_vy_state(VX), 2) + powf(vx_vy_state(VY), 2));
...
*  Function : Evaluate when a ship keeping constant speed and course will
                          arrive at location x,y. Assumes that x,y is on the
line.
*  Author   : Sverre Velten Rothmund
*  Modified :
*****************************************************************************************/
inline double evaluate_arrival_time(const Eigen::Vector4d &ship, const double x,
                                    const double y) {
  ...
  // 0/inf can happen if res.x-line(PX) ~=0. Avoid this by using the biggest of
  // x and y to evaluate
  double dx = x - ship(PX);
  double dy = y - ship(PY);
