	...
	namespace CPU
	{
		...
		) const
		{
			...
			for (int section = 0; section < 10; ++section) {
				...
				for (int k = section * samples_per_section; k < (section + 1) * samples_per_section; k += 1) {
					t = pars.dt * k;
				...
				total_max_cost += max_cost_in_section;  // Accumulate the max cost of each section
			}

			average_max_cost = total_max_cost / 10;  // Calculate the average of the max costs of the 10 sections
