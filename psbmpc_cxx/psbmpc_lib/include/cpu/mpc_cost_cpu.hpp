...
// Transitional variables are used  by the SBMPC to implement the transitional
// cost in Hagen, 2018.
struct Transitional_Variables {
  // Transitional indicator variables at the current time in addition to
  // <obstacle ahead> (AH_0) and <obstacle is passed> (IP_0) indicators
  std::vector<bool> AH_0, S_TC_0, S_i_TC_0, O_TC_0, Q_TC_0, IP_0, H_TC_0,
      X_TC_0;
...
private:
  ...
  inline double Delta_u(const double u_1, const double u_2) const {
    return pars.K_du * fabs(u_1 - u_2);
  }
  ...
  int find_triplet_orientation(const Eigen::Vector2d &p,
                               const Eigen::Vector2d &q,
                               const Eigen::Vector2d &r) const;

  bool determine_if_on_segment(const Eigen::Vector2d &p,
                               const Eigen::Vector2d &q,
                               const Eigen::Vector2d &r) const;

  bool determine_if_behind(const Eigen::Vector2d &p_1,
                           const Eigen::Vector2d &v_1,
                           const Eigen::Vector2d &v_2,
                           const double d_to_line) const;

  bool determine_if_lines_intersect(const Eigen::Vector2d &p_1,
                                    const Eigen::Vector2d &q_1,
                                    const Eigen::Vector2d &p_2,
                                    const Eigen::Vector2d &q_2) const;

  double distance_to_line(const Eigen::Vector2d &p, const Eigen::Vector2d &q_1,
                          const Eigen::Vector2d &q_2) const;

  double distance_to_static_obstacle(const Eigen::Vector2d &p,
                                     const Eigen::Vector2d &v_1,
                                     const Eigen::Vector2d &v_2) const;

public:
  bool determine_if_inside_polygon(const Eigen::Vector2d &p,
                                   const polygon_2D &poly) const;

  Eigen::Vector2d distance_to_line_segment(const Eigen::Vector2d &p,
                                           const Eigen::Vector2d &q_1,
                                           const Eigen::Vector2d &q_2) const;

  Eigen::Vector2d distance_to_polygon(const Eigen::Vector2d &p,
                                      const polygon_2D &poly) const;
  ...
  /* MPC_Cost(const Parameters &pars, const std::map<int,
     COLREGS_Violation_Evaluator> colregs_violation_evaluators) : pars(pars),
     colregs_violation_evaluators(colregs_violation_evaluators) {} */

  MPC_Cost &operator=(const MPC_Cost &other) = default;

  bool determine_transitional_cost_indicator(
      const Transitional_Variables &tv, const double psi_A, const double psi_B,
      const Eigen::Vector2d &L_AB, const double chi_m, const int i) const;

  bool determine_COLREGS_violation(const Eigen::Vector2d &v_A,
                                   const double psi_A,
  ...
  double calculate_dynamic_obstacle_cost(const Eigen::VectorXd &h_do_i_ps,
                                         const Dynamic_Obstacles &obstacles,
                                         const int i);

  // This one is used in the CPU PSBMPC for cost plotting
  double calculate_dynamic_obstacle_cost(Eigen::VectorXd &h_do_i_ps,
  ...
  // CPU PSBMPC cost
  double calculate_dynamic_obstacle_cost(const Eigen::MatrixXd &trajectory,
                                         const Eigen::MatrixXd &P_c_i,
                                         const Dynamic_Obstacles &obstacles,
                                         const int i,
                                         const double ownship_length) const;
  ...
  inline double calculate_collision_cost(const Eigen::Vector2d &v_1,
                                         const Eigen::Vector2d &v_2) const {
    return pars.K_coll * pow((v_1 - v_2).norm(), 2);
  }

  double calculate_ad_hoc_collision_risk(const double d_AB,
                                         const double t) const;

  double
  calculate_control_deviation_cost(const Eigen::VectorXd &offset_sequence,
                                   const double u_m_last,
                                   const double chi_m_last) const;

  double calculate_control_deviation_cost(
      const Eigen::VectorXd &offset_sequence, const double u_m_last,
      const double chi_m_last, const double max_cross_track_error) const;

  double
  calculate_grounding_cost(const Eigen::MatrixXd &trajectory,
                           const Eigen::Matrix<double, 4, -1> &static_obstacles,
                           const double ownship_length) const;

  double calculate_grounding_cost(const Eigen::MatrixXd &trajectory,
                                  const Static_Obstacles &polygons,
                                  const double V_w,
                                  const Eigen::Vector2d &wind_direction) const;

  double calculate_grounding_cost(Eigen::VectorXd &max_cost_j,
                                  const Eigen::MatrixXd &trajectory,
                                  const Static_Obstacles &polygons,
                                  const double V_w,
                                  const Eigen::Vector2d &wind_direction) const;

  double calculate_grounding_cost_section_based(
      const Eigen::MatrixXd &trajectory, const Static_Obstacles &polygons,
      const double V_w, const Eigen::Vector2d &wind_direction) const;

  void update_colregs_violation_node(const Eigen::Vector4d &ownship_state,
  ...
  double
  calculate_colregs_violation_cost(const Eigen::MatrixXd &ownship_trajectory,
                                   const Eigen::MatrixXd &obstacle_trajectory,
                                   const Eigen::VectorXd &offset_sequence,
                                   const double &Pr_WGW, const double &Pr_CCEM,
                                   const int i) {
    colregs_violation_evaluators.at(i).reset();
    double d_0i_0 = evaluateDistance(
        ownship_trajectory.col(0),
        obstacle_trajectory.col(0)); // In: Distance at the current time t_0
                                     // between ownship and obstacle i
    double d_cpa = INFINITY; // In: Distance at predicted CPA between ownship
                             // and obstacle i
    ...
    for (auto t = 0;
         t < std::min(ownship_trajectory.cols(), obstacle_trajectory.cols());
         ++t) {
      const double current_distance = evaluateDistance(
          ownship_trajectory.col(t), obstacle_trajectory.col(t));
      ...
      colregs_violation_evaluators.at(i).evaluate_predicted_maneuver_changes(
          ownship_trajectory(2, t), ownship_trajectory(3, t));
    }

    if (colregs_violation_evaluators.count(i)) {
      bool so_violation =
          colregs_violation_evaluators.at(i).evaluate_SO_violation(d_0i_0);
      bool gw_violation =
          colregs_violation_evaluators.at(i).evaluate_GW_violation(
              ownship_CPA_state, obstacle_CPA_state_vx_vy, d_cpa);
      bool readily_apparant_violation =
          colregs_violation_evaluators.at(i)
              .evaluate_readily_apparent_violation(offset_sequence[1],
                                                   offset_sequence[0]);
      return pars.kappa_SO * so_violation * Pr_WGW +
             pars.kappa_GW * gw_violation * Pr_CCEM +
             pars.kappa_RA * readily_apparant_violation;
    } else {
      throw std::runtime_error(
          "Attempting to evaluate colregs violation for an uninitialized ship");
  ...
  calculate_colregs_violation_cost(const Eigen::MatrixXd &ownship_trajectory,
    ...
    for (const auto &obstacle : obstacles) {
      update_colregs_violation_node(ownship_trajectory.col(0),
                                    obstacle.get_trajectories()[0].col(0),
                                    obstacle.get_ID());
      ...
      for (size_t i = 0; i < trajs.size(); ++i) {
        total_cost += probabilities(i) * calculate_colregs_violation_cost(
                                             ownship_trajectory, trajs[i],
                                             offset_sequence, Pr_WGW, Pr_CCEM,
                                             obstacle.get_ID());
      }
    }
    return total_cost;
  ...
  double calculate_colregs_violation_cost(const Eigen::VectorXd &h_colregs_i_ps,
                                          const Dynamic_Obstacles &obstacles,
                                          const int i) const {
 ...
 *  Function : Determine if a transitional cost should be applied for the
 * current control behavior, using the method in Hagen, 2018. Used in the SBMPC
 *  Author   : Trym Tengesdal
 *  Modified :
 *****************************************************************************************/
template <typename Parameters>
bool MPC_Cost<Parameters>::determine_transitional_cost_indicator(
    const Transitional_Variables
        &tv, // In: Struct of all current time obstacle transitional variables
    const double psi_A, // In: Heading of vessel A
    const double psi_B, // In: Heading of vessel B
    const Eigen::Vector2d
        &L_AB,          // In: LOS vector pointing from vessel A to vessel B
    const double chi_m, // In: Candidate course offset currently followed
    const int i         // In: Index of obstacle
) const {
  bool S_TC(false), S_i_TC(false), O_TC(false), Q_TC(false), X_TC(false),
      H_TC(false);
  ...
  // For ownship overtaking the obstacle: Check if obstacle is on opposite side
  // of ownship to what was observed at t0
  if (!tv.S_TC_0[i]) {
  ...
  // For obstacle overtaking the ownship: Check if ownship is on opposite side
  // of obstacle to what was observed at t0
  if (!tv.S_i_TC_0[i]) {
...
bool MPC_Cost<Parameters>::determine_COLREGS_violation(
  ...
  A_is_overtaken = v_A.dot(v_B) > cos(pars.phi_OT) * v_A.norm() * v_B.norm() &&
                   v_A.norm() < v_B.norm() && v_A.norm() > 0.25;

  B_is_overtaken = v_B.dot(v_A) > cos(pars.phi_OT) * v_B.norm() * v_A.norm() &&
                   v_B.norm() < v_A.norm() && v_B.norm() > 0.25;

  B_is_starboard = angle_difference_pmpi(atan2(L_AB(1), L_AB(0)), psi_A) > 0;

  is_passed = ((v_A.dot(L_AB) < cos(112.5 * DEG2RAD) *
                                    v_A.norm() && // Vessel A's perspective
                !A_is_overtaken) ||
               (v_B.dot(-L_AB) < cos(112.5 * DEG2RAD) *
                                     v_B.norm() && // Vessel B's perspective
                !B_is_overtaken)) &&
              d_AB > pars.d_safe;

  is_head_on = v_A.dot(v_B) < -cos(pars.phi_HO) * v_A.norm() * v_B.norm() &&
               v_A.norm() > 0.25 && v_B.norm() > 0.25 && is_ahead;

  is_crossing = v_A.dot(v_B) < cos(pars.phi_CR) * v_A.norm() * v_B.norm() &&
                v_A.norm() > 0.25 && v_B.norm() > 0.25 && !is_head_on &&
                !is_passed;

  return is_close && ((B_is_starboard && is_head_on) ||
                      (B_is_starboard && is_crossing && !A_is_overtaken));
}
...
double MPC_Cost<Parameters>::calculate_dynamic_obstacle_cost(
    const Eigen::VectorXd
        &h_do_i_ps, // In: Max cost wrt obstacle i in prediction scenario ps,
                    // and the control behaviour from the calling loop
...
template <typename Parameters>
double MPC_Cost<Parameters>::calculate_dynamic_obstacle_cost(
    Eigen::VectorXd &h_do_i_ps, // In/Out: Max dynamic obstacle cost associated
                                // with the current control behaviour, wrt
                                // obstacle i in prediction scenario ps
    const Eigen::MatrixXd
        &trajectory, // In: Own-ship trajectory when following the current
                     // offset_sequence/control behaviour
    const Eigen::MatrixXd
        &P_c_i, // In: Predicted obstacle collision probabilities for all
                // prediction scenarios, n_ps[i] x n_samples
    const Dynamic_Obstacles &obstacles, // In: Dynamic obstacle information
    const int i,                        // In: Index of obstacle
    const double
        ownship_length // In: Length of the ownship along the body x-axis
) const {
  ...
  for (int k = 0; k < n_samples; k++) {
    ...
    for (int ps = 0; ps < n_ps; ps++) {
      ...
      // Decrease the distance between the vessels by their respective max
      // dimension
      ...
      // Should discount time when using a prediction scheme where the
      // uncertainty for each obstacle prediction scenario is bounded by r_ct
      cost_ps = l_i * cost_coll * P_c_i(ps, k) *
                exp(-(double)k * pars.dt / pars.T_coll);
      ...
      /* if (ps == 1)
              printf("k = %d | C = %.4f | P_c_i = %.6f | mu = %d | v_i_p = %.2f,
         %.2f | psi_0_p = %.2f | v_0_p = %.2f, %.2f | d_0i_p = %.2f | L_0i_p =
         %.2f, %.2f\n", k, cost_coll, P_c_i(ps, k), mu, v_i_p(0), v_i_p(1),
         psi_0_p, v_0_p(0), v_0_p(1), d_0i_p, L_0i_p(0), L_0i_p(1)); */
    }
  }
...
double MPC_Cost<Parameters>::calculate_dynamic_obstacle_cost(
    const Eigen::MatrixXd
        &trajectory, // In: Own-ship trajectory when following the current
                     // offset_sequence/control behaviour
    const Eigen::VectorXd &offset_sequence, // In: Offset sequence currently
                                            // followed by the own-ship
    const Eigen::VectorXd
        &maneuver_times, // In: Time of each maneuver in the offset sequence
    const Dynamic_Obstacles &obstacles, // In: Dynamic obstacle information
    const Transitional_Variables
        &tv, // In: Struct of all current time obstacle transitional variables
    const int i, // In: Index of obstacle
  ...
  for (int k = 0; k < n_samples; k++) {
    ...
    if (k > 0) {
      COLREGS_violation_indicator =
          determine_COLREGS_violation(v_0_p, psi_0_p, v_i_p, L_0i_p, d_0i_p);
    }

    TC_indicator = determine_transitional_cost_indicator(tv, psi_0_p, psi_i_p,
                                                         L_0i_p, chi_m, i);
    ...
    cost = l_i * C * R + pars.kappa * COLREGS_violation_indicator +
           pars.kappa_TC * TC_indicator;
...
double MPC_Cost<Parameters>::calculate_ad_hoc_collision_risk(
    const double d_AB, // In: Distance between vessel A (typically the own-ship)
                       // and vessel B (typically an obstacle) 	   reduced by half
                       // the length of the two vessels (or only own-ship if
                       // static obstacles are considered)
...
double MPC_Cost<Parameters>::calculate_control_deviation_cost(
    const Eigen::VectorXd &offset_sequence, // In: Offset_sequence currently
                                            // followed by the own-ship
  ...
  for (int i = 0; i < pars.n_M; i++) {
    if (i == 0) {
      cost += pars.K_u * fabs(1 - offset_sequence[0]) +
              Delta_u(offset_sequence[0], u_m_last) +
              K_chi(offset_sequence[1]) +
              Delta_chi(offset_sequence[1], chi_m_last);
    } else {
      cost += pars.K_u * fabs(1 - offset_sequence[2 * i]) +
              Delta_u(offset_sequence[2 * i], offset_sequence[2 * i - 2]) +
              K_chi(offset_sequence[2 * i + 1]) +
              Delta_chi(offset_sequence[2 * i + 1], offset_sequence[2 * i - 1]);
    }
  }
  /* printf("K_u (1 - u_m_0) = %.4f | Delta_u(u_m_0, u_m_last) = %.4f |
     K_chi(chi_0) = %.4f | Delta_chi(chi_0, chi_last) = %.4f\n", pars.K_u * (1 -
     offset_sequence[0]), Delta_u(offset_sequence[0], u_m_last),
...
double MPC_Cost<Parameters>::calculate_control_deviation_cost(
    ...
    const double u_m_last,   // In: Previous optimal output surge modification
    const double chi_m_last, // In: Previous optimal output course modification
    const double max_cross_track_error // In: Maximum absolute predicted cross
                                       // track error
) const {
  double cost = 0.0;
  for (int i = 0; i < pars.n_M; i++) {
    if (i == 0) {
      cost += pars.K_u * fabs(1.0 - offset_sequence[0]) +
              Delta_u(offset_sequence[0], u_m_last) +
              pars.K_chi * pow(offset_sequence[1], 2) +
              pars.K_dchi *
                  pow(wrap_angle_to_pmpi(offset_sequence[1] - chi_m_last), 2);
    } else {
      cost += pars.K_u * fabs(1.0 - offset_sequence[2 * i]) +
              Delta_u(offset_sequence[2 * i], offset_sequence[2 * i - 2]) +
              pars.K_chi * pow(offset_sequence[2 * i + 1], 2) +
              pars.K_dchi * pow(wrap_angle_to_pmpi(offset_sequence[2 * i + 1] -
                                                   offset_sequence[2 * i - 1]),
                                2);
    }
  }
  /* printf("K_u (1 - u_m_0) = %.4f | Delta_u(u_m_0, u_m_last) = %.4f |
...
/****************************************************************************************
 *  Name     : calculate_grounding_cost_section_based
 *  Function : Determines penalty due to grounding ownship on static obstacles
 * (no-go zones) Author   : Modified :
 *****************************************************************************************/
template <typename Parameters>
double MPC_Cost<Parameters>::calculate_grounding_cost_section_based(
    ...
    const Eigen::Vector2d &wind_direction // In: Unit vector in NE describing
                                          // the estimated wind direction
) const {
  int n_samples = std::round(pars.T / pars.dt);
  int samples_per_section = n_samples / 10;
  ...
  double total_max_cost(0.0), average_max_cost(0.0);

  for (int section = 0; section < 10; ++section) {
    double max_cost_in_section = 0.0; // Reset the max cost for each section
    // std::cout << "section outside: " << section << std::endl;

    for (int k = section * samples_per_section;
         k < (section + 1) * samples_per_section; k += 1) {
      t = pars.dt * k;
      // std::cout << "k: " << k << std::endl;
      // std::cout << "section inside: " << section << std::endl;
      ...
      for (int j = 0; j < n_so; j++) {
        ...
        if (d_0j >= pars.d_safe) {
          cost_g = (pars.G_1 + pars.G_2 * phi_j * pow(V_w, 2)) *
                   exp(-(pars.G_3 * fabs(d_0j - pars.d_safe) + pars.G_4 * t));
        } else {
          cost_g_of_d_safe =
              (pars.G_1 + pars.G_2 * phi_j * pow(V_w, 2)) *
              exp(-pars.G_4 * t); // Using d_0j = d_safe to find a
          cost_g_der_of_d_safe =
              -pars.G_3 * (pars.G_1 + pars.G_2 * phi_j * pow(V_w, 2)) *
              exp(-pars.G_4 * t); // The derivative of cost_g_of_d_safe

          a = cost_g_der_of_d_safe; // a in y = a * x + b,
          b = cost_g_of_d_safe -
              a * pars.d_safe; // b in y = a * x + b <-> cost_g_of_d_safe  = a *
                               // d_safe + b

          cost_g =
              a * d_0j + b; // Linear cost function growth (a * d_0j) for d_0j <
                            // d_safe, b ensures a smooth transition between the
                            // exponential and linear area of the cost function

          // cost_g = (pars.G_1 + pars.G_2 * phi_j * pow(V_w, 2)) *
          // exp(-pars.G_4 * t); // Old cost function ("saturated", flat as a
          // function of d_0j after d_0j < d_safe)
        }

        if (max_cost_in_section < cost_g) {
          // d_0j_at_max_cost_g = d_0j;
          max_cost_in_section = cost_g;
        }
        // printf("t = %.2f | d_0j = %.6f | cost_g = %.6f | max_cost_g =
        // %.6f\n", k * pars.dt, d_0j, cost_g, max_cost_g);
      }
    }
    total_max_cost +=
        max_cost_in_section; // Accumulate the max cost of each section
  }

  average_max_cost =
      total_max_cost /
      10; // Calculate the average of the max costs of the 10 sections

  return average_max_cost;
}

/****************************************************************************************
 *  Name     : calculate_grounding_cost
 *  Function : Determines penalty due to grounding ownship on static obstacles
 * (no-go zones) Author   : Trym Tengesdal & Tom Daniel Grande Modified :
 *****************************************************************************************/
template <typename Parameters>
double MPC_Cost<Parameters>::calculate_grounding_cost(
  ...
  for (int k = 0; k < n_samples; k += pars.p_step_grounding) {
    ...
    for (int j = 0; j < n_so; j++) {
      ...
      if (d_0j < d_max_zero_g_cost) {
        ...
        cost_g_of_d_safe = (pars.G_1 + pars.G_2 * phi_j * pow(V_w, 2)) *
                           exp(-pars.G_4 * t); // Using d_0j = d_safe to find a
        cost_g_der_of_d_safe =
            -pars.G_3 * (pars.G_1 + pars.G_2 * phi_j * pow(V_w, 2)) *
            exp(-pars.G_4 * t); // The derivative of cost_g_of_d_safe
        ...
        // cost_g = (pars.G_1 + pars.G_2 * phi_j * pow(V_w, 2)) * exp(-pars.G_4
        // * t); // Old cost function ("saturated", flat as a function of d_0j
        // after d_0j < d_safe)
      }
      ...
      // The total cost of the given trajectory
      // total_cost_g += cost_g;
  ...
  // Average cost of the given trajectory
  // average_cost_g = total_cost_g/n_samples;
  // std::cout << "d_0j when maximum grounding cost " << d_0j_at_max_cost_g <<
  // std::endl;
  return max_cost_g;
}
...
template <typename Parameters>
double MPC_Cost<Parameters>::calculate_grounding_cost(
    Eigen::VectorXd &max_cost_j,       // In/Out: Max costs wrt to each static
                                       // obstacle, for debugging purposes
    const Eigen::MatrixXd &trajectory, // In: Predicted ownship trajectory
    const Static_Obstacles &polygons,  // In: Static obstacle information
    const double V_w,                  // In: Estimated wind speed
    const Eigen::Vector2d &wind_direction // In: Unit vector in NE describing
  ...
  for (int k = 0; k < n_samples; k += pars.p_step_grounding) {
    ...
    for (int j = 0; j < n_so; j++) {
      ...
      if (d_0j >= pars.d_safe) {
        ...
        cost_g =
            (pars.G_1 + pars.G_2 * phi_j * pow(V_w, 2)) * exp(-pars.G_4 * t);
      }
...
/****************************************************************************************
 *  Name     : calculate_grounding_cost
 *  Function : Determines penalty due grounding ownship on static obstacles
 * (no-go zones) Author   : Trym Tengesdal & Giorgio D. Kwame Minde Kufoalor
 *  Modified :
 *****************************************************************************************/
template <typename Parameters>
double MPC_Cost<Parameters>::calculate_grounding_cost(
    ...
    const Eigen::Matrix<double, 4, -1>
        &static_obstacles, // In: Static obstacle information
    const double
        ownship_length // In: Length of the ownship along the body x-axis
) const {
  ...
  for (int j = 0; j < n_so; j++) {
    ...
    if (!is_geo_constraint) {
      is_geo_constraint = determine_if_lines_intersect(p_0, p_1, so_1, so_2) ||
                          determine_if_behind(p_1, so_1, so_2, d_geo);
      break;
    }
  }
...
int MPC_Cost<Parameters>::find_triplet_orientation(
    ...
    const Eigen::Vector2d &r) const {
  double epsilon =
      1e-8; // abs(val) less than 1e-8 m^2 is considered zero for this check
 ...
 *			  (really if q is inside the rectangle with diagonal
 * pr...) Author   : Modified :
 *****************************************************************************************/
template <typename Parameters>
...
bool MPC_Cost<Parameters>::determine_if_behind(
    const Eigen::Vector2d &p_1, const Eigen::Vector2d &v_1,
    const Eigen::Vector2d &v_2, const double distance_to_line) const {
 ...
 *  Function : Determine if the line segments defined by p_1, q_1 and p_2, q_2
 * intersects Author   : Giorgio D. Kwame Minde Kufoalor Modified : By Trym
 * Tengesdal for more readability
 *****************************************************************************************/
template <typename Parameters>
bool MPC_Cost<Parameters>::determine_if_lines_intersect(
    const Eigen::Vector2d &p_1, const Eigen::Vector2d &q_1,
    const Eigen::Vector2d &p_2, const Eigen::Vector2d &q_2) const {
...
template <typename Parameters>
double
MPC_Cost<Parameters>::distance_to_line(const Eigen::Vector2d &p,
                                       const Eigen::Vector2d &q_1,
                                       const Eigen::Vector2d &q_2) const {
  Eigen::Vector3d a;
  Eigen::Vector3d b;
 ...
 *  Function : Calculate distance from p to obstacle defined by line segment
 * {v_1, v_2} Author   : Giorgio D. Kwame Minde Kufoalor Modified : By Trym
 * Tengesdal for more readability
 *****************************************************************************************/
template <typename Parameters>
double MPC_Cost<Parameters>::distance_to_static_obstacle(
    const Eigen::Vector2d &p, const Eigen::Vector2d &v_1,
    const Eigen::Vector2d &v_2) const {
  double d2line = distance_to_line(p, v_1, v_2);

  if (determine_if_behind(p, v_1, v_2, d2line) ||
      determine_if_behind(p, v_2, v_1, d2line))
    return d2line;
  else
...
bool MPC_Cost<Parameters>::determine_if_inside_polygon(
    const Eigen::Vector2d &p, const polygon_2D &poly) const {
//======================================================
...
//======================================================
  ...
  for (auto it = boost::begin(boost::geometry::exterior_ring(poly));
       it != boost::end(boost::geometry::exterior_ring(poly)) - 1; it++) {
  ...
  if (n_vertices < 3 || p(0) < bbox(0, 0) || p(0) > bbox(0, 1) ||
      p(1) < bbox(1, 0) || p(1) > bbox(1, 1)) {
    return false;
  }
...
//======================================================
  ...
  /* p_os_ray.col(1) = p_ray_end;
          ...
          Eigen::Map<Eigen::MatrixXd> map_polygon_matrix(p_polygon_matrix, 2,
     n_vertices); map_polygon_matrix = vertices; map_bbox = bbox; map_p_os_ray =
     p_os_ray;
...
//======================================================
    ...
    if (determine_if_lines_intersect(p, p_ray_end, v, v_next)) {
      ...
      if (find_triplet_orientation(p, v, p_ray_end) == 0) {
        // Determine if:
        // 1) both polygon sides connected to v are below/above the ray: 0 or 2
        // intersections
        ...
        // add one extra to account for the fact that the other side connecting
        // the vertex will be checked, when (v_next = v).
        line_intersect_count += 1;
      }
    }
  }
...
Eigen::Vector2d MPC_Cost<Parameters>::distance_to_line_segment(
    const Eigen::Vector2d &p, const Eigen::Vector2d &q_1,
    const Eigen::Vector2d &q_2) const {
...
template <typename Parameters>
Eigen::Vector2d
MPC_Cost<Parameters>::distance_to_polygon(const Eigen::Vector2d &p,
                                          const polygon_2D &poly) const {
