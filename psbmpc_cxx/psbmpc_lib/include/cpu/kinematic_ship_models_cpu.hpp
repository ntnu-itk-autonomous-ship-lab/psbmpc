...
enum Path_Prediction_Shape {
  SMOOTH, // Standard implementation of the predicted paths in the PSB-MPC. The
          // generated shapes resembles a U (for one seq. avoidance man.)
  LINEAR  // "Piecewise-linear" prediction geometry. The generated shapes
          // resembles a V (for one seq. avoidance man.)
};
...
public:
  ...
  Kinematic_Ship(const double l, const double w, const double T_U,
                 const double T_chi, const double R_a, const double LOS_LD,
  ...
  void determine_active_waypoint_segment(
      const Eigen::Matrix<double, 2, -1> &waypoints, const Eigen::Vector4d &xs);

  void update_guidance_references(double &u_d, double &chi_d,
                                  const Eigen::Matrix<double, 2, -1> &waypoints,
                                  const Eigen::Vector4d &xs, const double dt,
  ...
  void update_guidance_references(double &u_d, double &chi_d, const double e_m,
                                  const Eigen::Matrix<double, 2, -1> &waypoints,
                                  const Eigen::Vector4d &xs, const double dt);

  Eigen::Vector2d
  update_guidance_references_py(double &u_d, double &chi_d,
                                const Eigen::Matrix<double, 2, -1> &waypoints,
                                const Eigen::Vector4d &xs, const double dt,
                                const Guidance_Method guidance_method);

  Eigen::Vector3d update_guidance_references_py(
      double &u_d, double &chi_d, double cross_track_error,
      const Eigen::Matrix<double, 2, -1> &waypoints, const Eigen::Vector4d &xs,
      const double dt, const Guidance_Method guidance_method);

  Eigen::Vector2d
  update_guidance_references_py(double &u_d, double &chi_d, const double e_m,
                                const Eigen::Matrix<double, 2, -1> &waypoints,
                                const Eigen::Vector4d &xs, const double dt);

  Eigen::Vector4d predict(const Eigen::Vector4d &xs_old, const double U_d,
                          const double chi_d, const double dt,
                          const Prediction_Method prediction_method);

  void predict_trajectory(Eigen::MatrixXd &trajectory,
                          const Eigen::VectorXd &offset_sequence,
                          const Eigen::VectorXd &maneuver_times,
                          const double u_d, const double chi_d,
                          const Eigen::Matrix<double, 2, -1> &waypoints,
                          const Prediction_Method prediction_method,
                          const Guidance_Method guidance_method, const double T,
                          const double dt);
  ...
  void predict_trajectory(Eigen::MatrixXd &trajectory, const double e_m,
                          const double u_d, const double chi_d,
                          const Eigen::Matrix<double, 2, -1> &waypoints,
                          const Prediction_Method prediction_method,
  ...
  void predict_trajectory(Eigen::MatrixXd &trajectory, const double e_m,
                          const double chi_m, const double u_d,
                          const double chi_d,
                          const Eigen::Matrix<double, 2, -1> &waypoints,
                          const Prediction_Method prediction_method,
                          const Path_Prediction_Shape path_prediction_shape,
  ...
  Eigen::MatrixXd predict_trajectory_py(
      Eigen::MatrixXd &trajectory, const Eigen::VectorXd &offset_sequence,
      const Eigen::VectorXd &maneuver_times, const double u_d,
      const double chi_d, const Eigen::Matrix<double, 2, -1> &waypoints,
      const Prediction_Method prediction_method,
      const Guidance_Method guidance_method, const double T, const double dt);

  Eigen::MatrixXd predict_trajectory_py(
      Eigen::MatrixXd &trajectory, double &max_cross_track_error,
      const Eigen::VectorXd &offset_sequence,
      const Eigen::VectorXd &maneuver_times, const double u_d,
      const double chi_d, const Eigen::Matrix<double, 2, -1> &waypoints,
      const Prediction_Method prediction_method,
  ...
  Eigen::MatrixXd
  predict_trajectory_py(Eigen::MatrixXd &trajectory, const double e_m,
                        const double u_d, const double chi_d,
                        const Eigen::Matrix<double, 2, -1> &waypoints,
                        const Prediction_Method prediction_method,
                        const double T, const double dt);
  ...
  Eigen::MatrixXd predict_trajectory_py(
      ...
      const Prediction_Method prediction_method,
      const Guidance_Method guidance_method,
      const Path_Prediction_Shape path_prediction_shape, const double T,
      const double dt);
