		...
		struct Basic_Polygon
		{
			...
			__host__ Basic_Polygon &operator=(const polygon_2D &poly)
			{
				...
				for (auto it = boost::begin(boost::geometry::exterior_ring(poly)); it != boost::end(boost::geometry::exterior_ring(poly)) - 1; it++)
				{
					...
					if (vertices(0, v_count) < bbox(0, 0))
					{
						bbox(0, 0) = vertices(0, v_count);
					} // x_min
					if (vertices(1, v_count) < bbox(1, 0))
					{
						bbox(1, 0) = vertices(1, v_count);
					} // y_min
					if (vertices(0, v_count) > bbox(0, 1))
					{
						bbox(0, 1) = vertices(0, v_count);
					} // x_max
					if (vertices(1, v_count) > bbox(1, 1))
					{
						bbox(1, 1) = vertices(1, v_count);
					} // y_max
					v_count += 1;
				}
				return *this;
