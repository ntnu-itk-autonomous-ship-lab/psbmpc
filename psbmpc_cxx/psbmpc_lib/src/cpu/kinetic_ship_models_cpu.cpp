...
namespace PSBMPC_LIB
{
	namespace CPU
	{
		...
		*****************************************************************************************/
		...
		{
			Dvv(0) = -D_l(0, 0) * nu(0) - D_l(0, 1) * nu(1) - D_l(0, 2) * nu(2) - X_uu * fabs(nu(0)) * nu(0) - X_uuu * nu(0) * nu(0) * nu(0);

			Dvv(1) = -D_l(1, 0) * nu(0) - D_l(1, 1) * nu(1) - D_l(1, 2) * nu(2) - Y_vv * fabs(nu(1)) * nu(1) - Y_rv * fabs(nu(2)) * nu(1) - Y_vr * fabs(nu(1)) * nu(2) - Y_rr * fabs(nu(2)) * nu(2) - Y_vvv * nu(1) * nu(1) * nu(1);

			Dvv(2) = -D_l(2, 0) * nu(0) - D_l(2, 1) * nu(1) - D_l(2, 2) * nu(2) - N_vv * fabs(nu(1)) * nu(1) - N_rv * fabs(nu(2)) * nu(1) - N_vr * fabs(nu(1)) * nu(2) - N_rr * fabs(nu(2)) * nu(2) - N_rrr * nu(2) * nu(2) * nu(2);
		}
		...
		Telemetron::Telemetron()
		{
			...
			X_uu = -135.0;
			Y_vv = -2000.0;
			Y_vr = 0.0, Y_rv = 0.0, Y_rr = 0.0;
			N_vv = 0.0;
			N_vr = 0.0, N_rv = 0.0, N_rr = 0.0;
			X_uuu = 0.0;
			Y_vvv = 0.0;
			N_rrr = -3224.0;
		...
		)
		{
			...
			for (int k = 0; k < n_samples; k++)
			{
				...
				update_ctrl_input(u_m * u_d_p, CPU::wrap_angle_to_pmpi(chi_m + chi_d_p), xs);
		...
		void Telemetron::predict_trajectory(
			Eigen::MatrixXd &trajectory,				   // In/out: Own-ship trajectory
			double &max_cross_track_error,				   // In/out: Maximum absolute predicted cross track error
			const Eigen::VectorXd &offset_sequence,		   // In: Sequence of offsets in the candidate control behavior
			const Eigen::VectorXd &maneuver_times,		   // In: Time indices for each ownship avoidance maneuver
			const double u_d,							   // In: Surge reference
			const double chi_d,							   // In: Course reference
		...
		)
		{
			...
			int man_count = 0;
			double u_m = 1, u_d_p = u_d, e_k(0.0);
			double chi_m = 0, chi_d_p = chi_d;
			Eigen::Matrix<double, 6, 1> xs = trajectory.col(0);
