...
int SBMPC_Parameters::get_ipar(
  ...
  default:
    std::cout << "Index " << index << " is not a valid int parameter index."
              << std::endl;
    return 0;
  }
}
...
double SBMPC_Parameters::get_dpar(
  ...
  case i_dpar_epsilon_rdp_SBMPC:
    return epsilon_rdp;
  default:
    std::cout << "Index " << index << " is not a valid double parameter index."
              << std::endl;
    return 0.0;
  }
}
...
std::vector<Eigen::VectorXd> SBMPC_Parameters::get_opar(
    const int index // In: Index of parameter to return (Must be of
                    // std::vector<Eigen::VectorXd> type)
) const {
  ...
  default: {
    ...
    std::cout << "Index " << index << " is not a valid vector parameter index."
              << std::endl;
    return bs;
  }
  }
...
void SBMPC_Parameters::set_par(const int index, // In: Index of parameter to set
...
void SBMPC_Parameters::set_par(
  ...
  n_M = (int)value.size(); // Overrides the n_M value (changes if n_M is not set
                           // in accordance with value arg)
  ...
  if (!is_obstacle_sbmpc) // Tuning for SB-MPC
  {
    ...
    for (int M = 0; M < n_M; M++) {
      if (M == 0) {
        ...
        chi_offsets[M] << -90.0, -75.0, -60.0, -45.0, -30.0, -15.0, 0.0, 15.0,
        ...
        // chi_offsets[M] << -90.0, -75.0, -60.0, -45.0, -30.0, -15.0,
...
void SBMPC_Parameters::initialize_pars(
    const std::vector<std::vector<double>>
        &u_offsets, // In: Vector of vectors of surge modifications
    const std::vector<std::vector<double>>
        &chi_offsets, // In: Vector of vectors of course modifications
  ...
  assert((int)u_offsets.size() > 0 && (int)chi_offsets.size() > 0 && n_M > 0 &&
         n_do_ps > 0);
