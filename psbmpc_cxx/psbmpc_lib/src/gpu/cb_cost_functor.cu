 ...
 */
		...
		printf("        %.1f, %.1f, %.1f, %.1f\n", P_i_p(12, n_seg_samples - 1), P_i_p(13, n_seg_samples - 1), P_i_p(14, n_seg_samples - 1), P_i_p(15, n_seg_samples - 1)); */
			...
			//==================================================================================================
			if (offset_sequence(0) > 0.9 && offset_sequence(0) < 1.1 && offset_sequence(1) > -91.0 * M_PI / 180.0 && offset_sequence(1) < -89.0 * M_PI / 180.0 &&
				offset_sequence(2) > -0.1 && offset_sequence(2) < 0.1 &&
				offset_sequence(3) > -61.0 * M_PI / 180.0 && offset_sequence(3) < -59.0 * M_PI / 180.0)
			{
				...
				printf("Thread %d | i = %d | ps = %d | cb index %d | h_do_i_ps : %.4f| h_colregs_i_ps : %.4f | Pr_WGW = %.4f | Pr_CCEM = %.4f | cb : %.1f, %.1f, %.1f, %.1f \n", thread_index, i, ps, cb_index, h_do_i_ps, h_colregs_i_ps, Pr_WGW, Pr_CCEM, offset_sequence(0), RAD2DEG * offset_sequence(1), offset_sequence(2), RAD2DEG * offset_sequence(3));
			}

			/* if (offset_sequence(1) > 0.0)
			{
				printf("Thread %d | i = %d | ps = %d | cb index %d | h_do_i_ps : %.4f| h_colregs_i_ps : %.4f | Pr_WGW = %.4f | Pr_CCEM = %.4f | cb : %.1f, %.1f, %.1f, %.1f \n", thread_index, i, ps, cb_index, h_do_i_ps, h_colregs_i_ps, Pr_WGW, Pr_CCEM, offset_sequence(0), RAD2DEG * offset_sequence(1), offset_sequence(2), RAD2DEG * offset_sequence(3));
			} */
			//==================================================================================================
