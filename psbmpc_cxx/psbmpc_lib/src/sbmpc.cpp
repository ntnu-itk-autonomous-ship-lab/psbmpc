...
SBMPC::SBMPC(
    ...
    const SBMPC_Parameters
        &pars // In: Parameter object to initialize the SB-MPC
    )
...
SBMPC::SBMPC(const std::shared_ptr<CPU::Kinematic_Ship>
                 &ownship_ptr, // In/out from/to Python: Own-ship with specific
                               // parameter set
             const SBMPC_Parameters
                 &pars // In: Parameter object to initialize the SB-MPC
             )
...
void SBMPC::calculate_optimal_offsets(
    ...
    Eigen::MatrixXd
        &predicted_trajectory, // In/out: Predicted optimal ownship trajectory
    ...
    const Eigen::Matrix<double, 4, -1>
        &static_obstacles, // In: Static obstacle information parameterized as
                           // no-go lines
  ...
  ownship.predict_trajectory(trajectory, offset_sequence, maneuver_times, u_d,
                             chi_d, waypoints, pars.prediction_method,
                             pars.guidance_method, pars.T, pars.dt);
  ...
  for (int cb = 0; cb < pars.n_cbs; cb++) {
    ...
    for (int i = 0; i < n_do; i++) {
      cost_i(i) = mpc_cost.calculate_dynamic_obstacle_cost(
          trajectory, offset_sequence, maneuver_times, obstacles, tv, i,
          ownship.get_length());
    }

    cost += cost_i.maxCoeff();

    // cost += mpc_cost.calculate_grounding_cost(trajectory, static_obstacles,
    // ownship.get_length());

    cost += mpc_cost.calculate_control_deviation_cost(offset_sequence,
                                                      u_opt_last, chi_opt_last);
...
for (int M = 0; M < pars.n_M; M++)
{
  std::cout << opt_offset_sequence(2 * M) << ", " << opt_offset_sequence(2 * M +
1) * RAD2DEG; if (M < pars.n_M - 1) std::cout << ", ";
}
std::cout << std::endl;
...
void SBMPC::calculate_optimal_offsets(
    ...
    const Static_Obstacles
        &polygons, // In: Static obstacles parameterized as polygons
    const Dynamic_Obstacles &obstacles, // In: Dynamic obstacle information
    const bool disable // In: Disable the COLAV functionality or not
) {
  int n_samples = std::round(pars.T / pars.dt);
  ...
  for (int cb = 0; cb < pars.n_cbs; cb++) {
    ...
    cost += mpc_cost.calculate_grounding_cost(trajectory, polygons, V_w,
                                              wind_direction);
  ...
  std::cout << "Optimal offset sequence : ";
  for (int M = 0; M < pars.n_M; M++) {
    std::cout << opt_offset_sequence(2 * M) << ", "
              << opt_offset_sequence(2 * M + 1) * RAD2DEG;
    if (M < pars.n_M - 1)
      std::cout << ", ";
  }
  std::cout << std::endl;
...
optimal_offsets_results_SBMPC_py SBMPC::calculate_optimal_offsets_py(
    ...
    const std::vector<Eigen::MatrixXd>
        &polygons_py, // In: Static obstacles parameterized as polygons
    const Dynamic_Obstacles &obstacles,  // In: Dynamic obstacle information
    const bool new_static_obstacle_data, // In: Flag to indicate if new static
                                         // obstacle data is given
    const bool disable // In: Disable the COLAV functionality or not
) {
  // u_opt, chi_opt and predicted_trajectory moved from method inputs, now used
  // in the output instead of void
  double u_opt;   // Out: Optimal surge offset
  double chi_opt; // Out: Optimal course offset
  Eigen::MatrixXd
      predicted_trajectory; // Out: Predicted optimal ownship trajectory

  // Making the Python polygons compatible with the C++ codebase's
  // Static_Obstacles
  ...
  maneuver_times.setZero();
  ownship_ptr->predict_trajectory(trajectory, offset_sequence, maneuver_times,
                                  u_d, chi_d, waypoints, pars.prediction_method,
                                  pars.guidance_method, pars.T, pars.dt);
  ...
  for (int cb = 0; cb < pars.n_cbs; cb++) {
    ...
    ownship_ptr->predict_trajectory(
        trajectory, offset_sequence, maneuver_times, u_d, chi_d, waypoints,
        pars.prediction_method, pars.guidance_method, pars.T, pars.dt);

    for (int i = 0; i < n_do; i++) {
      cost_i(i) = mpc_cost.calculate_dynamic_obstacle_cost(
          trajectory, offset_sequence, maneuver_times, obstacles, tv, i,
          ownship_ptr->get_length());
    }
  ...
  // for (int M = 0; M < pars.n_M; M++)
  //{
  //	std::cout << opt_offset_sequence(2 * M) << ", " << opt_offset_sequence(2
  //* M + 1) * RAD2DEG; 	if (M < pars.n_M - 1) 		std::cout << ", ";
  // }
  // std::cout << std::endl;
...
void SBMPC::increment_control_behaviour() {
  for (int M = pars.n_M - 1; M > -1; M--) {
    // Only increment counter for "leaf node offsets" on each iteration, which
    // are the course offsets in the last maneuver
    if (M == pars.n_M - 1) {
      offset_sequence_counter(2 * M + 1) += 1;
    }

    // If one reaches the end of maneuver M's course offsets, reset
    // corresponding counter and increment surge offset counter above
    if (offset_sequence_counter(2 * M + 1) == pars.chi_offsets[M].size()) {
      offset_sequence_counter(2 * M + 1) = 0;
      offset_sequence_counter(2 * M) += 1;
    }
    offset_sequence(2 * M + 1) =
        pars.chi_offsets[M](offset_sequence_counter(2 * M + 1));
...
void SBMPC::update_transitional_variables(
  ...
  for (int i = 0; i < n_do; i++) {
    ...
    // std::cout << "Obst i = " << i << " ahead at t0 ? " << tv.AH_0[i] <<
    // std::endl;

    // Obstacle on starboard side
    tv.S_TC_0[i] =
        CPU::angle_difference_pmpi(atan2(L_AB(1), L_AB(0)), psi_A) > 0;

    // std::cout << "Obst i = " << i << " on starboard side at t0 ? " <<
    // tv.S_TC_0[i] << std::endl;

    // Ownship on starboard side of obstacle
    tv.S_i_TC_0[i] = atan2(-L_AB(1), -L_AB(0)) > psi_B;

    // std::cout << "Own-ship on starboard side of obst i = " << i << " at t0 ?
    // " << tv.S_i_TC_0[i] << std::endl;

    // Ownship overtaking the obstacle
    tv.O_TC_0[i] = v_B.dot(v_A) > cos(pars.phi_OT) * v_B.norm() * v_A.norm() &&
                   v_B.norm() < v_A.norm() && v_B.norm() > 0.25 && is_close &&
                   tv.AH_0[i];

    // std::cout << "Own-ship overtaking obst i = " << i << " at t0 ? " <<
    // tv.O_TC_0[i] << std::endl;

    // Obstacle overtaking the ownship
    tv.Q_TC_0[i] = v_A.dot(v_B) > cos(pars.phi_OT) * v_A.norm() * v_B.norm() &&
                   v_A.norm() < v_B.norm() && v_A.norm() > 0.25 && is_close &&
                   !tv.AH_0[i];

    // std::cout << "Obst i = " << i << " overtaking the ownship at t0 ? " <<
    // tv.Q_TC_0[i] << std::endl;

    // Determine if the obstacle is passed by
    tv.IP_0[i] = ((v_A.dot(L_AB) < cos(112.5 * DEG2RAD) *
                                       v_A.norm() && // Ownship's perspective
                   !tv.Q_TC_0[i]) ||
                  (v_B.dot(-L_AB) < cos(112.5 * DEG2RAD) *
                                        v_B.norm() && // Obstacle's perspective
                   !tv.O_TC_0[i])) &&
                 d_AB > pars.d_safe;

    // std::cout << "Obst i = " << i << " passed by at t0 ? " << tv.IP_0[i] <<
    // std::endl;

    // This is not mentioned in article, but also implemented here..
    tv.H_TC_0[i] = v_A.dot(v_B) < -cos(pars.phi_HO) * v_A.norm() * v_B.norm() &&
                   v_A.norm() > 0.25 && v_B.norm() > 0.25 && tv.AH_0[i];

    // std::cout << "Head-on at t0 wrt obst i = " << i << " ? " << tv.H_TC_0[i]
    // << std::endl;
    ...
    tv.X_TC_0[i] = v_A.dot(v_B) < cos(pars.phi_CR) * v_A.norm() * v_B.norm() &&
                   !tv.H_TC_0[i] && !tv.IP_0[i] && v_A.norm() > 0.25 &&
                   v_B.norm() > 0.25;

    // std::cout << "Crossing at t0 wrt obst i = " << i << " ? " << tv.X_TC_0[i]
    // << std::endl;
  }
}
...
void SBMPC::setup_prediction(
  ...
  for (int M = 1; M < pars.n_M; M++) {
    // If a predicted collision occurs with the closest obstacle, avoidance
    // maneuver M is taken right after the obstacle possibly maneuvers (modelled
    // to be at t_0 + M * t_ts given that t_cpa > t_ts. If t_cpa < t_ts or n_do
    // = 0, the subsequent maneuver is taken at t_{M-1} + t_ts + 1 anyways
    // (simplification)
    maneuver_times(M) =
        maneuver_times(M - 1) + std::round((pars.t_ts + 1) / pars.dt);

    // Otherwise, find the closest obstacle (wrt t_cpa) that is a possible
    // hazard
    t_cpa_min = 1e10;
    index_closest = -1;
    for (int i = 0; i < n_do; i++) {
      ...
      d_safe_i = pars.d_safe +
                 0.5 * (ownship.get_length() + obstacles[i].get_length());
      // For the current avoidance maneuver, determine which obstacle that
      // should be considered, i.e. the closest obstacle that is not already
      // passed (which means that the previous avoidance maneuver happened
      // before CPA with this obstacle)
      if (!maneuvered_by[i] && maneuver_times(M - 1) * pars.dt < t_cpa(i) &&
          t_cpa(i) <= t_cpa_min && t_cpa(i) <= pars.T) {
        t_cpa_min = t_cpa(i);
        index_closest = i;
      }
    }
    ...
    if (index_closest != -1) {
      d_safe_i = pars.d_safe + 0.5 * (ownship.get_length() +
                                      obstacles[index_closest].get_width());
      ...
      if (d_cpa(index_closest) > d_safe_i) {
        // std::cout << "OS maneuver M = " << M << " at t = " <<
        // t_cpa(index_closest) << " wrt obstacle " << index_closest <<
        // std::endl;
  ...
  // std::cout << "Ownship maneuver times = " << maneuver_times.transpose() <<
  // std::endl;
}
...
void SBMPC::setup_prediction_ptr_ver(
  ...
  for (int M = 1; M < pars.n_M; M++) {
    ...
    for (int i = 0; i < n_do; i++) {
      ...
      d_safe_i = pars.d_safe +
                 0.5 * (ownship_ptr->get_length() + obstacles[i].get_length());
      // For the current avoidance maneuver, determine which obstacle that
      // should be considered, i.e. the closest obstacle that is not already
      // passed (which means that the previous avoidance maneuver happened
      // before CPA with this obstacle)
    ...
    if (index_closest != -1) {
      d_safe_i = pars.d_safe + 0.5 * (ownship_ptr->get_length() +
                                      obstacles[index_closest].get_width());
      // If no predicted collision,  avoidance maneuver M with the closest
      // obstacle (that is not passed) is taken at t_cpa_min
      if (d_cpa(index_closest) > d_safe_i) {
...
Static_Obstacles SBMPC::process_list_of_np_polygons(
  ...
  for (const Eigen::MatrixXd &polygon_py : polygons_py) {
    for (int i = 0; i < polygon_py.rows() - 1; ++i) {
      for (int j = 0; j < polygon_py.cols(); ++j) {
        // std::cout << "x: " << polygon_py(i, j) << "y: " << polygon_py(i + 1,
        // j) << std::endl;
        point_2D boost_point;
        boost::geometry::assign_values(boost_point, polygon_py(i, j),
                                       polygon_py(i + 1, j));
  ...
  // std::cout << "Number of static obstacles: " <<
  // boost_static_obstacles.size() << std::endl;
