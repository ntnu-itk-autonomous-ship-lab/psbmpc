/****************************************************************************************
*
*  File name : joint_prediction_manager.hpp
*
...
namespace PSBMPC_LIB
{
	...
	private:
		...
		std::vector<Obstacle_Data<CPU::Prediction_Obstacle>> data;
		...
		*****************************************************************************************/
			...
			) 			
		{
			...
			for (int j = 0; j < n_obst_new; j++)
			{
				...
				if (!obstacle_exist)
				{
					data[i].new_obstacles.push_back(std::move(CPU::Prediction_Obstacle(
			...
			)
		{
			...
			for (int j = 0; j < n_obst; j++)
			{
				...
				*********************************************************************/
				...
				data[i].S_TC_0[j] = CPU::angle_difference_pmpi(atan2(L_AB(1), L_AB(0)), psi_A) > 0;
	...
	public:
		...
		Obstacle_Data<CPU::Prediction_Obstacle>& get_data(int i) { return data[i]; };
		...
		*****************************************************************************************/
		...
		void operator()(
			const Parameter_Object &mpc_pars,													// In: Parameters of the obstacle manager boss, an Obstacle_SBMPC
			const std::vector<CPU::Prediction_Obstacle> &pobstacles, 							// In: Vector of Prediction Obstacles
