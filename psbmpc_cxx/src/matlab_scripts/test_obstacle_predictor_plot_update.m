...
delete(h_P_i_ps{ps});
h_P_i_ps{ps} = plot(ax1, X_i_ps(2, k) + r_ellipse(:, 2), ...
    X_i_ps(1, k) + r_ellipse(:, 1), 'c', 'Linewidth', 1.6); 

% IF using LOS prediction:
% Plot velocity components against mean mrou velocity components
%h_gt_Vx_i_ps{ps} = plot(ax2, t_vec(1:k), X_i_ps(4, 1:k) .* cos(X_i_ps(3, 1:k)), 'Linewidth', 1.6);

%h_gt_Vy_i_ps{ps} = plot(ax3, t_vec(1:k), X_i_ps(4, 1:k) .* sin(X_i_ps(3, 1:k)), 'Linewidth', 1.6);

% ELSE (MROU)
delete(h_gt_Vx_i_ps{ps}); delete(h_mean_Vx_i_ps{ps}); 
delete(h_gt_Vy_i_ps{ps}); delete(h_mean_Vy_i_ps{ps});
h_gt_Vx_i_ps{ps} = plot(ax2, t_vec(1:k), X_i_ps(3, 1:k), 'Linewidth', 1.6);
h_mean_Vx_i_ps{ps} = plot(ax2, t_vec(1:k), v_i_ps(1, 1:k), 'Linewidth', 1.6);

h_gt_Vy_i_ps{ps} = plot(ax3, t_vec(1:k), X_i_ps(4, 1:k), 'Linewidth', 1.6);
h_mean_Vy_i_ps{ps} = plot(ax3, t_vec(1:k), v_i_ps(2, 1:k), 'Linewidth', 1.6);
