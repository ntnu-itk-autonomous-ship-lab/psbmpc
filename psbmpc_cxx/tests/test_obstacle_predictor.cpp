...
TEST(ObstaclePredictorTest, ObstaclePrediction) {
#if ENABLE_TEST_FILE_PLOTTING
  ...
  waypoints_i << xs_i_0(0), 1000, xs_i_0(1), 0;
  ...
  oship.predict_trajectory(trajectory_i, offset_sequence, maneuver_times,
                           xs_i_0(2), xs_i_0(3), waypoints_i, PSBMPC_LIB::ERK1,
                           PSBMPC_LIB::LOS, T, dt);
  ...
  obstacles.push_back(PSBMPC_LIB::Tracked_Obstacle(
      xs_i_aug, PSBMPC_LIB::CPU::flatten(P_0), Pr_s, false, T, dt));
  ...
  for (int k = 0; k < n_samples; k++) {
    ...
    obstacles[0].update(xs_i_aug, PSBMPC_LIB::CPU::flatten(P_0), Pr_s, false,
                        dt);

    if (k % 20 == 0) {
      std::cout << "xs_i = " << xs_i_aug.block<4, 1>(0, 0).transpose()
                << std::endl;
      //==============================================
      ...
      for (int k_p = 0; k_p < n_samples; k_p += 10) {
        ...
        for (int ps = 0; ps < n_ps; ps++) {
          ...
          Eigen::Map<Eigen::MatrixXd> map_pred_traj(
              p_pred_traj, xs_p[ps].rows(), xs_p[ps].cols());
          Eigen::Map<Eigen::MatrixXd> map_v_traj(p_v_traj, v_p[0].rows(),
                                                 v_p[0].cols());
          Eigen::Map<Eigen::MatrixXd> map_P_traj(p_P_traj, P_p.rows(),
                                                 P_p.cols());
