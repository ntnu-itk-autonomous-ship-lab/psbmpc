...
// Main program:
//*****************************************************************************************************************
TEST(PSBMPCCostClassTest, DistanceToPolygon) {
#if ENABLE_TEST_FILE_PLOTTING
  // Matlab engine setup
  ...
  /* waypoints << 0, 200, 200, 400, 600,  300, 500,
                           0, 0,   200, 200,  0,  0, -200; */
  waypoints << xs_os_0(0), 7042350, xs_os_0(1), 270575;

  Eigen::VectorXd offset_sequence(2), maneuver_times(1);
  maneuver_times(0) = 0;
  offset_sequence << 1.0, 0.0;
  asv_sim.predict_trajectory(trajectory, offset_sequence, maneuver_times, u_d,
  ...
  PSBMPC_LIB::CPU::MPC_Cost<PSBMPC_LIB::PSBMPC_Parameters> mpc_cost_cpu(
      psbmpc.pars);
#if USE_GPU_PSBMPC
  PSBMPC_LIB::GPU::MPC_Cost<PSBMPC_LIB::PSBMPC_Parameters> mpc_cost_gpu(
      psbmpc.pars);
#endif

  // double V_w = 0.0;
  // Eigen::Vector2d wind_direction; wind_direction(0) = 1.0; wind_direction(1)
  // = 0.0;
  ...
  Eigen::Vector2d map_origin_ned(7042250, 270250);
  PSBMPC_LIB::Grounding_Hazard_Manager grounding_hazard_manager(
      filename, map_origin_ned, psbmpc.pars);
  ...
  Eigen::MatrixXd d2poly_cpu(2, polygons.size());
#if USE_GPU_PSBMPC
  Eigen::MatrixXd d2poly_gpu(2, polygons.size());
  TML::Vector2f v, p_os;
  Eigen::Vector2d e;
  ...
  for (int j = 0; j < n_static_obst; j++) {
    std::cout << "Boost dist to poly: "
              << boost::geometry::distance(p, polygons[j]) << std::endl;
    d2poly_cpu.col(j) = mpc_cost_cpu.distance_to_polygon(
        xs_os_0.block<2, 1>(0, 0), polygons[j]);
  ...
  for (auto it =
           boost::begin(boost::geometry::exterior_ring(selected_polygons[0]));
       it != boost::end(boost::geometry::exterior_ring(selected_polygons[0]));
       ++it) {
  ...
  Eigen::Map<Eigen::MatrixXd> map_selected_polygon_matrix(
      p_selected_polygon_matrix, 1, 2);
