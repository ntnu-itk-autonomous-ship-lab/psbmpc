	...
	)
{
	...
	for (int i = 0; i < n_obst; i++)
	{
		...
		*********************************************************************/
		...
		AH_0[i] = v_A.dot(L_AB) > cos(psbmpc_pars.phi_AH) * v_A.norm();
		...
		O_TC_0[i] = v_B.dot(v_A) > cos(psbmpc_pars.phi_OT) * v_B.norm() * v_A.norm() 	&&
			  	v_B.norm() < v_B.norm()							    		&&
				v_B.norm() > 0.25											&&
				is_close 													&&
				AH_0[i];
		...
		Q_TC_0[i] = v_A.dot(v_B) > cos(psbmpc_pars.phi_OT) * v_A.norm() * v_B.norm() 	&&
				v_A.norm() < v_B.norm()							  			&&
				v_A.norm() > 0.25 											&&
				is_close 													&&
				!AH_0[i];
